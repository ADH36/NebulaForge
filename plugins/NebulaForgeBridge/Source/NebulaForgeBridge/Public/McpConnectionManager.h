...
class UNebulaForgeBridgeSettings;
...
/**
 * Manages WebSocket connections for the NebulaForge Bridge.
 * Handles listening, connecting, reconnecting, heartbeats, and message dispatching.
 */
class NEBULAFORGEBRIDGE_API FMcpConnectionManager : public TSharedFromThis<FMcpConnectionManager>
{
public:
	FMcpConnectionManager();
	~FMcpConnectionManager();

	void Initialize(const UNebulaForgeBridgeSettings* Settings);
	void Start();
	void Stop();
