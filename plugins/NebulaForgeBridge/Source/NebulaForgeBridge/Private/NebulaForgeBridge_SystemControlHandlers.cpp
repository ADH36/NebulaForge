...
bool UNebulaForgeBridgeSubsystem::HandleSystemControlAction(
  ...
  const bool bGetTimeSecondsAction = Lower == TEXT("get_time_seconds");
  ...
  const bool bGetCurrentReverbEffectAction = Lower == TEXT("get_current_reverb_effect");
  const bool bGetPlayerControllerAction = Lower == TEXT("get_player_controller");
  const bool bGetPlayerPawnAction = Lower == TEXT("get_player_pawn");
  const bool bGetPlayerCharacterAction = Lower == TEXT("get_player_character");
  const bool bGetPlayerCameraManagerAction = Lower == TEXT("get_player_camera_manager");
  const bool bGetPlayerStateAction = Lower == TEXT("get_player_state");
  ...
  if (bGetAvailableSpatialPluginsAction) {
    TArray<TSharedPtr<FJsonValue>> Plugins;
    for (const FName& PluginName : UGameplayStatics::GetAvailableSpatialPluginNames(GetWorld())) {
      Plugins.Add(MakeShared<FJsonValueString>(PluginName.ToString()));
    }
    TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
    Result->SetArrayField(TEXT("plugins"), Plugins);
    SendAutomationResponse(RequestingSocket, RequestId, true, TEXT("Available spatial audio plugins read"), Result, FString());
    return true;
  }

  if (bGetPlatformNameAction) {
    TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
  ...
  if (bGetTimeSecondsAction || bGetUnpausedTimeSecondsAction || bGetRealTimeSecondsAction) {
    double TimeSeconds = 0.0;
    if (bGetTimeSecondsAction) {
      TimeSeconds = UGameplayStatics::GetTimeSeconds(GetWorld());
    } else if (bGetUnpausedTimeSecondsAction) {
      TimeSeconds = UGameplayStatics::GetUnpausedTimeSeconds(GetWorld());
    } else {
      TimeSeconds = UGameplayStatics::GetRealTimeSeconds(GetWorld());
    }
    TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
    Result->SetNumberField(TEXT("timeSeconds"), TimeSeconds);
    SendAutomationResponse(RequestingSocket, RequestId, true, TEXT("World time read"), Result, FString());
    return true;
  }

  if (bGetCurrentLevelNameAction) {
    bool bRemovePrefixString = true;
    Payload->TryGetBoolField(TEXT("removePrefixString"), bRemovePrefixString);
    TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
  ...
  if (bGetCurrentReverbEffectAction) {
    UReverbEffect* ReverbEffect = UGameplayStatics::GetCurrentReverbEffect(GetWorld());
    TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
    Result->SetBoolField(TEXT("hasReverbEffect"), ReverbEffect != nullptr);
    Result->SetStringField(TEXT("reverbEffectPath"), ReverbEffect ? ReverbEffect->GetPathName() : FString());
    SendAutomationResponse(RequestingSocket, RequestId, true, TEXT("Current reverb effect read"), Result, FString());
    return true;
  }

  if (bGetPlayerControllerAction || bGetPlayerPawnAction || bGetPlayerCharacterAction || bGetPlayerCameraManagerAction || bGetPlayerStateAction) {
    int32 PlayerIndex = 0;
    Payload->TryGetNumberField(TEXT("playerIndex"), PlayerIndex);
    if (PlayerIndex < 0 || PlayerIndex > 32) {
      SendAutomationError(RequestingSocket, RequestId, TEXT("playerIndex must be between 0 and 32"), TEXT("INVALID_ARGUMENT"));
      return true;
    }
    UObject* PlayerObject = nullptr;
    if (bGetPlayerControllerAction) PlayerObject = UGameplayStatics::GetPlayerController(GetWorld(), PlayerIndex);
    else if (bGetPlayerPawnAction) PlayerObject = UGameplayStatics::GetPlayerPawn(GetWorld(), PlayerIndex);
    else if (bGetPlayerCharacterAction) PlayerObject = UGameplayStatics::GetPlayerCharacter(GetWorld(), PlayerIndex);
    else if (bGetPlayerCameraManagerAction) PlayerObject = UGameplayStatics::GetPlayerCameraManager(GetWorld(), PlayerIndex);
    else PlayerObject = UGameplayStatics::GetPlayerState(GetWorld(), PlayerIndex);
    TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
    Result->SetNumberField(TEXT("playerIndex"), PlayerIndex);
    Result->SetBoolField(TEXT("found"), PlayerObject != nullptr);
    Result->SetStringField(TEXT("objectPath"), PlayerObject ? PlayerObject->GetPathName() : FString());
    Result->SetStringField(TEXT("classPath"), PlayerObject ? PlayerObject->GetClass()->GetPathName() : FString());
    const TCHAR* LookupLabel = bGetPlayerControllerAction ? TEXT("Player controller") : bGetPlayerPawnAction ? TEXT("Player pawn") : bGetPlayerCharacterAction ? TEXT("Player character") : bGetPlayerCameraManagerAction ? TEXT("Player camera manager") : TEXT("Player state");
