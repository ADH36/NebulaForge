// =============================================================================
// NebulaForgeBridge_BlueprintHandlers.cpp
// =============================================================================
...
#include "NebulaForgeBridge_BlueprintCreationHandlers.h"
#include "NebulaForgeBridge_SCSHandlers.h"
...
static void FNebulaForgeBridge_LogConnectionFailure(const TCHAR *Context, UEdGraphPin *SourcePin, UEdGraphPin *TargetPin, const FPinConnectionResponse &Response);

static UEdGraphPin *
FNebulaForgeBridge_FindExecPin(UEdGraphNode *Node,
...
static UEdGraphPin *
FNebulaForgeBridge_FindPreferredEventExec(UEdGraph *Graph) {
  // Delegate to centralized McpBlueprintUtils
  return McpBlueprintUtils::FindPreferredEventExec(Graph);
}

static UEdGraphPin *
FNebulaForgeBridge_FindDataPin(UEdGraphNode *Node,
                                 EEdGraphPinDirection Direction,
...
FNebulaForgeBridge_CreateVariableGetter(UEdGraph *Graph,
...
static bool FNebulaForgeBridge_AttachValuePin(UK2Node_VariableSet *VarSet,
                                                UEdGraph *Graph,
  ...
  UEdGraphPin *ValuePin =
      FNebulaForgeBridge_FindDataPin(VarSet, EGPD_Input, VarMemberName);
  if (!ValuePin) {
    ValuePin =
        FNebulaForgeBridge_FindDataPin(VarSet, EGPD_Input, NAME_VarSetValue);
  }

  if (!ValuePin) {
    UE_LOG(
        LogNebulaForgeBridgeSubsystem, Verbose,
        TEXT("FNebulaForgeBridge_AttachValuePin: no Value pin found on %s"),
        *VarSet->GetName());
    return false;
  }
  ...
  auto TryLinkPins = [&](UEdGraphPin *SourcePin,
    ...
    if (Response.Response == CONNECT_RESPONSE_MAKE) {
      ...
      FNebulaForgeBridge_LogConnectionFailure(ContextLabel, SourcePin,
                                                ValuePin, Response);
    }
    return false;
  };
  ...
  for (UEdGraphNode *Node : Graph->Nodes) {
    ...
    if (UK2Node_VariableGet *VarGet = Cast<UK2Node_VariableGet>(Node)) {
      ...
      UEdGraphPin *GetValuePin =
          FNebulaForgeBridge_FindDataPin(VarGet, EGPD_Output, VarMemberName);
      if (!GetValuePin) {
        static const FName NAME_VarGetValue(TEXT("Value"));
        GetValuePin = FNebulaForgeBridge_FindDataPin(VarGet, EGPD_Output,
                                                       NAME_VarGetValue);
      }
      if (GetValuePin) {
        bLinkedFromExisting =
  ...
  if (!bOutLinked) {
    ...
    UK2Node_VariableGet *SpawnedGet = FNebulaForgeBridge_CreateVariableGetter(
        Graph, VarSet->VariableReference, VarSet->NodePosX - 250.0f,
        VarSet->NodePosY);
    if (SpawnedGet) {
      UEdGraphPin *SpawnedOutput = FNebulaForgeBridge_FindDataPin(
          SpawnedGet, EGPD_Output, VarMemberName);
      if (!SpawnedOutput) {
        static const FName NAME_SpawnValue(TEXT("Value"));
        SpawnedOutput = FNebulaForgeBridge_FindDataPin(
...
static bool FNebulaForgeBridge_EnsureExecLinked(UEdGraph *Graph) {
  if (!Graph) {
    return false;
  }
  ...
  UEdGraphPin *EventOutput = FNebulaForgeBridge_FindPreferredEventExec(Graph);
  if (!EventOutput) {
    return false;
  }
  ...
  for (UEdGraphNode *Node : Graph->Nodes) {
    ...
    if (Node->IsA<UK2Node_VariableSet>() || Node->IsA<UK2Node_CallFunction>()) {
      if (UEdGraphPin *ExecInput =
              FNebulaForgeBridge_FindExecPin(Node, EGPD_Input)) {
        if (ExecInput && ExecInput->LinkedTo.Num() == 0) {
          ...
          if (Response.Response == CONNECT_RESPONSE_MAKE) {
            ...
            FNebulaForgeBridge_LogConnectionFailure(
...
static void FNebulaForgeBridge_LogConnectionFailure(
...
static FEdGraphPinType FNebulaForgeBridge_MakePinType(const FString &InType) {
  // Delegate to centralized McpBlueprintUtils
...
static FString
FNebulaForgeBridge_JsonValueToString(const TSharedPtr<FJsonValue> &Value) {
  // Delegate to centralized McpHandlerUtils
  return McpHandlerUtils::JsonValueToString(Value);
}

static FName FNebulaForgeBridge_ResolveMetadataKey(const FString &RawKey) {
...
#if MCP_HAS_EDGRAPH_SCHEMA_K2
static void
FNebulaForgeBridge_AddUserDefinedPin(UK2Node *Node, const FString &PinName,
  ...
  const FEdGraphPinType PinTypeDesc = FNebulaForgeBridge_MakePinType(PinType);
...
static UFunction *
FNebulaForgeBridge_ResolveFunction(UBlueprint *Blueprint,
...
static FProperty *
FNebulaForgeBridge_FindProperty(UBlueprint *Blueprint,
...
static FString
FNebulaForgeBridge_DescribePinType(const FEdGraphPinType &PinType) {
  // Delegate to centralized McpBlueprintUtils
  return McpBlueprintUtils::DescribePinType(PinType);
}

static void FNebulaForgeBridge_AppendPinsJson(
  ...
  for (const TSharedPtr<FUserPinInfo> &PinInfo : Pins) {
    ...
    PinJson->SetStringField(
        TEXT("type"), FNebulaForgeBridge_DescribePinType(PinInfo->PinType));
    Out.Add(MakeShared<FJsonValueObject>(PinJson));
  }
}

static bool FNebulaForgeBridge_CollectVariableMetadata(
  ...
  if (Blueprint) {
    ...
    if (FProperty *Property = FNebulaForgeBridge_FindProperty(
...
FNebulaForgeBridge_BuildVariableJson(const UBlueprint *Blueprint,
                                       const FBPVariableDescription &VarDesc);

static FString
FNebulaForgeBridge_DescribePropertyType(const FProperty *Property) {
  if (!Property) {
    return FString();
  }
  ...
  if (const UEdGraphSchema_K2 *Schema = GetDefault<UEdGraphSchema_K2>()) {
    if (Schema->ConvertPropertyToPinType(Property, PinType)) {
      return FNebulaForgeBridge_DescribePinType(PinType);
    }
  }
#endif
...
static void FNebulaForgeBridge_AnnotateVariableJson(
...
FNebulaForgeBridge_BuildVariableJson(const UBlueprint *Blueprint,
  ...
  Obj->SetStringField(TEXT("type"),
                      FNebulaForgeBridge_DescribePinType(VarDesc.VarType));
  ...
  if (FNebulaForgeBridge_CollectVariableMetadata(Blueprint, VarDesc,
                                                   Metadata)) {
...
FNebulaForgeBridge_CollectBlueprintVariables(UBlueprint *Blueprint) {
  // Delegate to centralized McpBlueprintUtils
  return McpBlueprintUtils::CollectBlueprintVariables(Blueprint);
}

static TSharedPtr<FJsonObject> FNebulaForgeBridge_CollectBlueprintDefaults(
  ...
  for (const TSharedPtr<FJsonValue> &VariableValue : Variables) {
    ...
    FProperty *Property =
        FNebulaForgeBridge_FindProperty(Blueprint, VariableName);
...
static TArray<TSharedPtr<FJsonValue>>
FNebulaForgeBridge_CollectBlueprintFunctions(UBlueprint *Blueprint) {
  // Delegate to centralized McpBlueprintUtils
  return McpBlueprintUtils::CollectBlueprintFunctions(Blueprint);
}

static void
FNebulaForgeBridge_CollectEventPins(UK2Node *Node,
                                      TArray<TSharedPtr<FJsonValue>> &Out) {
  if (!Node) {
    return;
  }
  ...
  if (UK2Node_CustomEvent *CustomEvent = Cast<UK2Node_CustomEvent>(Node)) {
    FNebulaForgeBridge_AppendPinsJson(CustomEvent->UserDefinedPins, Out);
  } else if (UK2Node_FunctionEntry *FunctionEntry =
                 Cast<UK2Node_FunctionEntry>(Node)) {
    FNebulaForgeBridge_AppendPinsJson(FunctionEntry->UserDefinedPins, Out);
  }
}

static TArray<TSharedPtr<FJsonValue>>
FNebulaForgeBridge_CollectBlueprintEvents(UBlueprint *Blueprint) {
  TArray<TSharedPtr<FJsonValue>> Out;
  if (!Blueprint) {
    return Out;
  }
  ...
  auto AppendEvent = [&](const FString &EventName, const FString &EventType,
    ...
    FNebulaForgeBridge_CollectEventPins(SourceNode, Params);
    if (Params.Num() > 0) {
...
static TSharedPtr<FJsonObject>
FNebulaForgeBridge_FindNamedEntry(const TArray<TSharedPtr<FJsonValue>> &Array,
...
static TSharedPtr<FJsonObject>
FNebulaForgeBridge_EnsureBlueprintEntry(const FString &Key) {
...
static TSharedPtr<FJsonObject>
FNebulaForgeBridge_BuildBlueprintSnapshot(UBlueprint *Blueprint,
  ...
  TArray<TSharedPtr<FJsonValue>> Variables =
      FNebulaForgeBridge_CollectBlueprintVariables(Blueprint);
  TSharedPtr<FJsonObject> Defaults =
      FNebulaForgeBridge_CollectBlueprintDefaults(Blueprint, Variables);
  ...
  Snapshot->SetArrayField(
      TEXT("functions"),
      FNebulaForgeBridge_CollectBlueprintFunctions(Blueprint));
  Snapshot->SetArrayField(
      TEXT("events"), FNebulaForgeBridge_CollectBlueprintEvents(Blueprint));
...
static void DiagnosticPatternChecks(const FString &CleanAction,
  ...
  for (const TCHAR *P : Patterns) {
    ...
    UE_LOG(LogNebulaForgeBridgeSubsystem, VeryVerbose,
 ...
 */
bool UNebulaForgeBridgeSubsystem::HandleBlueprintAction(
      ...
      if (NormalizedType == TEXT("modify_component")) {
        ...
        if (!ComponentClass) {
          ...
          if (ExistingNode) {
            ...
            if (Subsystem) {
              ...
              using namespace NebulaForgeBridge;
        ...
        if (Subsystem) {
          ...
          if (bFound) {
            constexpr bool bHasDelete =
                NebulaForgeBridge::THasDeleteSubobject<
        ...
        if (Subsystem) {
          ...
          constexpr bool bHasAttach =
              NebulaForgeBridge::THasAttach<USubobjectDataSubsystem>::value;
  ...
  if (ActionMatchesPattern(TEXT("blueprint_set_variable_metadata")) ||
    ...
    for (const auto &Pair : MetaObjPtr->Values) {
      ...
      const FString ValueStr =
          FNebulaForgeBridge_JsonValueToString(Pair.Value);
      const FName MetaKey = FNebulaForgeBridge_ResolveMetadataKey(KeyStr);
    ...
    const TSharedPtr<FJsonObject> Snapshot =
        FNebulaForgeBridge_BuildBlueprintSnapshot(Blueprint, RegistryKey);
  ...
  if (ActionMatchesPattern(TEXT("blueprint_add_variable")) ||
    ...
    if (bAlreadyExists) {
      UE_LOG(
          LogNebulaForgeBridgeSubsystem, Log,
    ...
    if (Snapshot.IsValid()) {
      ...
      if (Snapshot->HasField(TEXT("variables"))) {
        ...
        if (const TSharedPtr<FJsonObject> VarJson =
                FNebulaForgeBridge_FindNamedEntry(Vars, TEXT("name"),
                                                    VarName)) {
          Response->SetObjectField(TEXT("variable"), VarJson);
        }
      }
  ...
  if (ActionMatchesPattern(TEXT("blueprint_add_event")) ||
    ...
    if (bIsCustomEvent) {
      ...
      if (CustomEventNode && Params.Num() > 0) {
        ...
        for (const TSharedPtr<FJsonValue> &ParamVal : Params) {
          ...
          // pins on the node)
          FNebulaForgeBridge_AddUserDefinedPin(CustomEventNode, ParamName,
    ...
    TSharedPtr<FJsonObject> Entry =
        FNebulaForgeBridge_EnsureBlueprintEntry(RegistryKey);
  ...
  if (ActionMatchesPattern(TEXT("blueprint_remove_event")) ||
    ...
    TSharedPtr<FJsonObject> Entry =
        FNebulaForgeBridge_EnsureBlueprintEntry(RegistryPath);
    TArray<TSharedPtr<FJsonValue>> Events =
        Entry->HasField(TEXT("events")) ? Entry->GetArrayField(TEXT("events"))
                                        : TArray<TSharedPtr<FJsonValue>>();
  ...
  if (ActionMatchesPattern(TEXT("blueprint_add_function")) ||
    ...
    for (const TSharedPtr<FJsonValue> &Value : Inputs) {
      ...
      FNebulaForgeBridge_AddUserDefinedPin(EntryNode, ParamName, ParamType,
                                             EGPD_Input);
    }
    ...
    for (const TSharedPtr<FJsonValue> &Value : Outputs) {
      ...
      FString ParamType;
      Obj->TryGetStringField(TEXT("type"), ParamType);
      FNebulaForgeBridge_AddUserDefinedPin(
  ...
  if ((ActionMatchesPattern(TEXT("blueprint_get")) ||
    ...
    if (bExists) {
      ...
      Entry = FNebulaForgeBridge_BuildBlueprintSnapshot(BP, Key);

      // Merge functions and events from registry
      TSharedPtr<FJsonObject> RegistryEntry =
          FNebulaForgeBridge_EnsureBlueprintEntry(Key);
  ...
  if (ActionMatchesPattern(TEXT("blueprint_add_node")) ||
    ...
    if (NodeTypeLower.Contains(TEXT("callfunction")) ||
      ...
      if (FuncNode && !FunctionName.IsEmpty()) {
        if (UFunction *FoundFunc =
                FNebulaForgeBridge_ResolveFunction(BP, FunctionName)) {
    ...
    if (Schema) {
      if (UK2Node_VariableSet *VarSet = Cast<UK2Node_VariableSet>(NewNode)) {
        ...
        VarSet->Modify();
        FNebulaForgeBridge_AttachValuePin(VarSet, TargetGraph, Schema,
                                            bValueLinked);

        // Connect the exec input to a custom event if available
        UEdGraphPin *ExecInput =
            FNebulaForgeBridge_FindExecPin(VarSet, EGPD_Input);
        if (ExecInput) {
          if (ExecInput->LinkedTo.Num() == 0) {
            ...
            for (UEdGraphNode *Node : TargetGraph->Nodes) {
              if (UK2Node_CustomEvent *Custom =
                ...
                if (Custom->CustomFunctionName == OnCustomName) {
                  EventOutput =
                      FNebulaForgeBridge_FindExecPin(Custom, EGPD_Output);
                  if (EventOutput) {
                    break;
                  }
                }
            ...
            if (!EventOutput) {
              EventOutput =
                  FNebulaForgeBridge_FindPreferredEventExec(TargetGraph);
            }
      ...
      if (!bExecLinked) {
        bExecLinked =
            FNebulaForgeBridge_EnsureExecLinked(TargetGraph) || bExecLinked;
      }
    }
  ...
  if (ActionMatchesPattern(TEXT("blueprint_set_metadata")) ||
    ...
    for (const auto& Pair :
      ...
      const FName MetaKey = FNebulaForgeBridge_ResolveMetadataKey(MetadataKey);
      FString MetaValue;
...
bool UNebulaForgeBridgeSubsystem::HandleSCSAction(
    const FString &RequestId, const FString &Action,
    const TSharedPtr<FJsonObject> &Payload,
    TSharedPtr<FMcpBridgeWebSocket> RequestingSocket) {
#if WITH_EDITOR
