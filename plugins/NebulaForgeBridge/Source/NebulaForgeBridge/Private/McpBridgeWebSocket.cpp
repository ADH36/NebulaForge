...
void FMcpBridgeWebSocket::Listen() {
  ...
  UE_LOG(LogNebulaForgeBridgeSubsystem, Log,
         TEXT("Spawning NebulaForge automation server thread for %s:%d"), *ListenHost,
         Port);
  Thread = FRunnableThread::Create(
      this, TEXT("FMcpBridgeWebSocketServerWorker"), 0, TPri_Normal);
  if (!Thread) {
    UE_LOG(LogNebulaForgeBridgeSubsystem, Error,
           TEXT("Failed to create server thread for NebulaForge automation bridge."));
    DispatchOnGameThread([WeakThis = SelfWeakPtr] {
      if (TSharedPtr<FMcpBridgeWebSocket> Pinned = WeakThis.Pin()) {
        Pinned->ConnectionErrorDelegate.Broadcast(
...
uint32 FMcpBridgeWebSocket::RunServer() {
  ...
  ListenSocket = SocketSubsystem->CreateSocket(
      NAME_Stream, TEXT("NebulaForgeBridgeListenSocket"), ProtocolName);
  if (!ListenSocket) {
    const FString ErrorMessage = DescribeSocketError(
        SocketSubsystem, TEXT("Failed to create listen socket"));
    UE_LOG(LogNebulaForgeBridgeSubsystem, Error, TEXT("%s"), *ErrorMessage);
  ...
  UE_LOG(LogNebulaForgeBridgeSubsystem, Log, TEXT("Listen socket created."));
  ...
  const UNebulaForgeBridgeSettings* Settings = GetDefault<UNebulaForgeBridgeSettings>();
  ...
  if (bIsLoopback) {
    ...
    if (!bResolvedHost && HostToBind.Equals(TEXT("::1"), ESearchCase::IgnoreCase)) {
      ...
      ListenSocket = SocketSubsystem->CreateSocket(
          NAME_Stream, TEXT("NebulaForgeBridgeListenSocket"), FName());
      if (!ListenSocket) {
        UE_LOG(LogNebulaForgeBridgeSubsystem, Error, TEXT("Failed to re-create IPv4 socket for fallback."));
        return 0;
      }
      ListenSocket->SetReuseAddr(true);
      ListenSocket->SetNonBlocking(false);
      UE_LOG(LogNebulaForgeBridgeSubsystem, Log, TEXT("Re-created socket for IPv4 fallback."));
    ...
    if (!bResolvedHost) {
      ...
      if (AddrInfoResult.ReturnCode == SE_NO_ERROR && AddrInfoResult.Results.Num() > 0) {
        ...
        if (bResolvedIsIpv6 != bIsIpv6Host) {
          ...
          ListenSocket = SocketSubsystem->CreateSocket(
              NAME_Stream, TEXT("NebulaForgeBridgeListenSocket"), NewProtocolName);
          if (!ListenSocket) {
            UE_LOG(LogNebulaForgeBridgeSubsystem, Error,
    ...
    if (bIsIpv6Host) {
      ...
      ListenSocket->SetReuseAddr(true);
      ListenSocket->SetNonBlocking(false);
      UE_LOG(LogNebulaForgeBridgeSubsystem, Log, TEXT("Re-created socket for IPv4 fallback (non-loopback path)."));
    }
  ...
  UE_LOG(LogNebulaForgeBridgeSubsystem, Log,
         TEXT("NebulaForge Bridge listening on %s"), *BoundAddress);
  ...
  while (!bStopping && ListenSocket) {
    ...
    FSocket *ClientSocket =
        ListenSocket->Accept(TEXT("NebulaForgeBridgeClient"));
...
bool FMcpBridgeWebSocket::PerformHandshake() {
  ...
  Socket = SocketSubsystem->CreateSocket(
      NAME_Stream, TEXT("NebulaForgeBridgeSocket"), false);
  if (!Socket) {
