...
export const MATERIAL_AUTHORING_ACTIONS = [
  'create_material', 'set_blend_mode', 'set_material_blend_mode', 'set_shading_model', 'set_material_shading_model', 'set_material_domain',
  ...
  'set_two_sided', 'set_material_two_sided',
] as const;
...
export const WIDGET_AUTHORING_ACTIONS = [
  ...
  'create_common_activatable_widget', 'create_common_button_base', 'create_common_tab_list', 'create_common_action_widget', 'configure_analog_cursor',
  ...
  'add_damage_indicator', 'create_inventory_ui', 'create_dialog_widget', 'create_radial_menu', 'play_dialogue', 'configure_photo_mode_ui',
...
export const consolidatedToolDefinitions: ToolDefinition[] = [
  ...
  },
  {
    name: 'terrain_data',
    category: 'world',
    description: 'Generate real-world terrain heightmaps, elevation previews, map images, and water features through the NebulaForge terrain API.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        position: { type: 'object', description: 'PaperTileMap local-space position as {x,y,z}.' },
        socketName: commonSchemas.socketName,
        alphaThreshold: { type: 'number', minimum: 0, maximum: 1, description: 'PaperSprite alpha threshold used for tight texture bounds.' },
        time: { type: 'number', description: 'Flipbook playback time in seconds.' },
        clampToEnds: { type: 'boolean', description: 'Clamp flipbook time to the first or last keyframe.' },
        frameIndex: { type: 'integer', minimum: 0, description: 'Flipbook frame index.' },
        ...
        layerColor: commonSchemas.objectProp,
        layerCollides: commonSchemas.booleanProp,
        overrideCollisionOffset: commonSchemas.booleanProp,
        collisionOffsetOverride: commonSchemas.numberProp,
        overrideCollisionThickness: commonSchemas.booleanProp,
        collisionThicknessOverride: commonSchemas.numberProp,
        renderInEditor: commonSchemas.booleanProp,
      ...
      properties: {
        action: {
          ...
          enum: [
            ...
            'configure_marker_widget', 'configure_marker_3d_2d', 'configure_marker_distance', 'configure_marker_occlusion', 'configure_photo_mode_camera', 'configure_photo_mode_poses', 'create_quest_manager', 'spawn_metahuman_actor', 'configure_metahuman_component', 'configure_avatar_component', 'setup_avatar_skeleton_mapping', 'apply_avatar_to_character', 'configure_avatar_lod', 'configure_metahuman_lod_bias', 'configure_groom_lod', 'configure_hair_simulation', 'set_hair_stiffness', 'set_hair_damping', 'configure_hair_collision', 'set_lod_settings', 'configure_hair_material', 'configure_body_type', 'set_body_type', 'get_face_parameters', 'set_face_parameter', 'set_skin_tone', 'set_eye_color', 'set_hair_color', 'set_hair_style', 'set_eyebrow_style', 'set_makeup', 'set_teeth_configuration', 'set_body_proportions', 'set_height', 'configure_lock_on_target', 'set_target_priority', 'configure_target_switching', 'configure_soft_lock', 'configure_aim_assist',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        keyStore: commonSchemas.stringProp,
        ...
        parameters: { type: 'object', maxProperties: 64, description: 'JSON-compatible parameters for Python execution.' }
      ,
        ...
        mass: commonSchemas.numberProp,
        massAsDensity: commonSchemas.booleanProp,
        enableClustering: commonSchemas.booleanProp,
        enableNanite: commonSchemas.booleanProp,
        supportRayTracing: commonSchemas.booleanProp,
        damageThresholds: { type: 'array', items: commonSchemas.numberProp, maxItems: 32 },
        maxClusterLevel: commonSchemas.numberProp,
        removeOnMaxSleep: commonSchemas.booleanProp,
        sourceAssetPath: commonSchemas.assetPath,
        enableDamageFromCollision: commonSchemas.booleanProp,
        clusterGroupIndex: commonSchemas.numberProp,
        maxSimulatedLevel: commonSchemas.numberProp,
        linearEtherDrag: commonSchemas.numberProp,
        collisionProfileName: commonSchemas.name,
        simulatePhysics: commonSchemas.booleanProp,
        variantSetName: { type: 'string', minLength: 1, maxLength: 128, description: 'Display name for a new Variant Manager variant set.' },
        variantName: { type: 'string', minLength: 1, maxLength: 128, description: 'Display name for a new Variant Manager variant.' },
        variantPropertyType: { type: 'string', enum: ['bool', 'int', 'float', 'string'], description: 'Scalar Variant Manager property setter type.' },
        variantPropertyValue: { type: 'string', maxLength: 4096, description: 'Scalar value captured by the Variant Manager property.' },
        dependencyVariantSetName: { type: 'string', minLength: 1, maxLength: 128, description: 'Dependency Variant Manager set name.' },
        dependencyVariantName: { type: 'string', minLength: 1, maxLength: 128, description: 'Dependency Variant Manager variant name.' },
        thumbnailSource: { type: 'string', enum: ['editor_viewport', 'file'], description: 'Variant thumbnail source.' },
        thumbnailPath: { type: 'string', maxLength: 4096, description: 'Project-relative image path for a Variant thumbnail.' },
        variantExportPath: { type: 'string', maxLength: 4096, description: 'Project-relative JSON output path for Variant Manager configuration metadata.' },
        uiScale: { type: 'number', minimum: 0.1, maximum: 4, description: 'Application-wide Slate/UMG scale.' },
        announcement: { type: 'string', minLength: 1, maxLength: 1024, description: 'Accessibility announcement to send to the platform.' }
      },
      required: ['action']
    },
    outputSchema: {
    ...
    inputSchema: {
      ...
      properties: {
        action: {
          type: 'string',
          enum: [
            'create', 'create_master_sequence', 'open', 'add_camera', 'create_cine_camera_actor', 'add_subsequence', 'add_shot_track', 'configure_shot_settings', 'inspect_shot_settings', 'add_audio_track', 'add_material_parameter_track', 'add_material_color_track', 'add_custom_primitive_data_track', 'add_niagara_system_track', 'create_niagara_float_parameter_track', 'add_niagara_float_parameter_key', 'add_niagara_integer_parameter_key', 'add_niagara_bool_parameter_key', 'add_niagara_vector_parameter_key', 'add_niagara_color_parameter_key', 'inspect_niagara_parameter_track', 'add_camera_cut_track', 'add_camera_shake_track', 'add_fade_track', 'add_level_visibility_track', 'add_skeletal_animation_track', 'add_transform_track', 'add_event_track', 'add_property_track', 'add_actor', 'add_actors', 'remove_actors',
        ...
        soundPath: commonSchemas.assetPath,
        looping: commonSchemas.booleanProp,
        ...
        colorA: commonSchemas.numberProp,
        vectorX: commonSchemas.numberProp,
        vectorY: commonSchemas.numberProp,
        vectorZ: commonSchemas.numberProp,
        shotIndex: commonSchemas.integerProp,
        shotDisplayName: commonSchemas.stringProp,
        thumbnailReferenceOffset: commonSchemas.numberProp,
            ...
            // Authoring status actions
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        // Common parameters
        name: commonSchemas.name,
        commandName: commonSchemas.name,
        commandSet: commonSchemas.name,
        commandContext: commonSchemas.name,
        commandLabel: commonSchemas.stringProp,
        commandDescription: commonSchemas.stringProp,
        overrideExisting: commonSchemas.booleanProp,
        projectPath: commonSchemas.stringProp,
        path: commonSchemas.directoryPathForCreation,
        savePath: commonSchemas.savePath,
        assetPath: commonSchemas.assetPath,
