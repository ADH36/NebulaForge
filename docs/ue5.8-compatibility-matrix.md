...
| Save/reload persistence | CONDITIONAL | Saves through project safe-save wrappers and verifies package existence and layer metadata. `reloadForVerification: true` performs a bounded safe map reload and rechecks the landscape actor/layer names; without it the result explicitly reports `reloadVerified: false`. |
