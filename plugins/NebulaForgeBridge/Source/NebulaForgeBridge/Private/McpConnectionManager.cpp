...
// (It is declared extern in NebulaForgeBridgeSubsystem.h)
...
void FMcpConnectionManager::Initialize(
    const UNebulaForgeBridgeSettings *Settings) {
  if (Settings) {
...
void FMcpConnectionManager::Start() {
  if (!TickerHandle.IsValid()) {
    ...
    const UNebulaForgeBridgeSettings *Settings =
        GetDefault<UNebulaForgeBridgeSettings>();
...
void FMcpConnectionManager::ForceReconnect(const FString &Reason,
  ...
  UE_LOG(LogNebulaForgeBridgeSubsystem, Warning, TEXT("ForceReconnect: %s"),
         *Reason);
