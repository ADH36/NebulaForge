...
| DataTables and CurveTables | ⚠️ | DataTable creation, transactional reflected row insertion/modification/deletion, and row readback are implemented; CurveTable rich-row authoring, readback, and CSV import/export are implemented, but simple-curve mode and advanced interpolation/tangent controls remain incomplete. |
