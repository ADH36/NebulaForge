...
static bool HandlePaperTileSetUVFromXYAction(
    UNebulaForgeBridgeSubsystem* Owner,
    const FString& RequestId,
    const TSharedPtr<FJsonObject>& Payload,
    TSharedPtr<FMcpBridgeWebSocket> Socket)
{
  ...
  int32 TileX = 0;
  int32 TileY = 0;
  if (!TileSet || !Payload->TryGetNumberField(TEXT("tileX"), TileX) || !Payload->TryGetNumberField(TEXT("tileY"), TileY) ||
      TileX < 0 || TileX >= TileSet->GetTileCountX() || TileY < 0 || TileY >= TileSet->GetTileCountY()) {
    Owner->SendAutomationError(Socket, RequestId, TEXT("assetPath must resolve to a PaperTileSet and tileX/tileY must be within the tile grid"), TEXT("INVALID_ARGUMENT"));
    return true;
  }
  const FIntPoint UV = TileSet->GetTileUVFromTileXY(FIntPoint(TileX, TileY));
  TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
  Result->SetStringField(TEXT("assetPath"), TileSet->GetPathName());
  ...
  Result->SetNumberField(TEXT("u"), UV.X);
  Result->SetNumberField(TEXT("v"), UV.Y);
  Owner->SendAutomationResponse(Socket, RequestId, true, TEXT("Paper tile-set tile UV read from coordinates"), Result, FString());
  return true;
}
...
static bool HandlePaperTileSetTileMetadataInspectAction(
    UNebulaForgeBridgeSubsystem* Owner,
    const FString& RequestId,
    const TSharedPtr<FJsonObject>& Payload,
    TSharedPtr<FMcpBridgeWebSocket> Socket)
{
  ...
  UPaperTileSet* TileSet = Cast<UPaperTileSet>(UEditorAssetLibrary::LoadAsset(AssetPath));
  int32 TileIndex = 0;
  const FPaperTileMetadata* Metadata = nullptr;
  if (!TileSet || !Payload->TryGetNumberField(TEXT("tileIndex"), TileIndex) || TileIndex < 0 || TileIndex >= TileSet->GetTileCount() ||
      (Metadata = TileSet->GetTileMetadata(TileIndex)) == nullptr) {
    Owner->SendAutomationError(Socket, RequestId, TEXT("assetPath must resolve to a PaperTileSet and tileIndex must be within its tile count"), TEXT("INVALID_ARGUMENT"));
    return true;
  }
  TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
...
static bool HandlePaperSpriteSocketsInspectAction(
    UNebulaForgeBridgeSubsystem* Owner,
    const FString& RequestId,
    const TSharedPtr<FJsonObject>& Payload,
    TSharedPtr<FMcpBridgeWebSocket> Socket)
{
  ...
  Sprite->QuerySupportedSockets(SocketDescriptions);
  TArray<TSharedPtr<FJsonValue>> Sockets;
  for (const FComponentSocketDescription& Description : SocketDescriptions) {
    TSharedPtr<FJsonObject> SocketObject = McpHandlerUtils::CreateResultObject();
    SocketObject->SetStringField(TEXT("name"), Description.Name.ToString());
    SocketObject->SetNumberField(TEXT("type"), static_cast<int32>(Description.Type));
  ...
  Owner->SendAutomationResponse(Socket, RequestId, true, TEXT("Paper sprite sockets inspected"), Result, FString());
  return true;
}
...
static bool HandlePaperSpriteSocketNamesValidateAction(
    UNebulaForgeBridgeSubsystem* Owner,
    const FString& RequestId,
    const TSharedPtr<FJsonObject>& Payload,
    TSharedPtr<FMcpBridgeWebSocket> Socket)
{
  ...
  for (const FComponentSocketDescription& Description : SocketDescriptions)
    Names.Add(MakeShared<FJsonValueString>(Description.Name.ToString()));
  bool bSave = true;
  Payload->TryGetBoolField(TEXT("save"), bSave);
  if (bSave && !McpSafeAssetSave(Sprite)) {
    Owner->SendAutomationError(Socket, RequestId, TEXT("Paper sprite socket names validated but save failed"), TEXT("SAVE_FAILED"));
    return true;
  }
  TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
  Result->SetStringField(TEXT("assetPath"), Sprite->GetPathName());
  Result->SetNumberField(TEXT("socketCount"), Names.Num());
  Result->SetArrayField(TEXT("socketNames"), Names);
  Result->SetBoolField(TEXT("saved"), bSave);
  Owner->SendAutomationResponse(Socket, RequestId, true, TEXT("Paper sprite socket names validated"), Result, FString());
  return true;
}

static bool HandlePaperFlipbookInspectAction(
    UNebulaForgeBridgeSubsystem* Owner,
    const FString& RequestId,
    const TSharedPtr<FJsonObject>& Payload,
    TSharedPtr<FMcpBridgeWebSocket> Socket)
{
  ...
  for (int32 Index = 0; Index < Flipbook->GetNumKeyFrames(); ++Index) {
    ...
    TSharedPtr<FJsonObject> KeyFrameObject = McpHandlerUtils::CreateResultObject();
    KeyFrameObject->SetNumberField(TEXT("index"), Index);
    KeyFrameObject->SetNumberField(TEXT("frameRun"), KeyFrame.FrameRun);
    KeyFrameObject->SetStringField(TEXT("spritePath"), KeyFrame.Sprite ? KeyFrame.Sprite->GetPathName() : FString());
    KeyFrames.Add(MakeShared<FJsonValueObject>(KeyFrameObject));
  }
  TSharedPtr<FJsonObject> Result = McpHandlerUtils::CreateResultObject();
  Result->SetStringField(TEXT("assetPath"), Flipbook->GetPathName());
  Result->SetNumberField(TEXT("framesPerSecond"), Flipbook->GetFramesPerSecond());
  Result->SetNumberField(TEXT("numKeyFrames"), Flipbook->GetNumKeyFrames());
  Result->SetNumberField(TEXT("numFrames"), Flipbook->GetNumFrames());
  Result->SetNumberField(TEXT("totalDuration"), Flipbook->GetTotalDuration());
  Result->SetNumberField(TEXT("collisionSource"), static_cast<int32>(Flipbook->GetCollisionSource()));
  Result->SetArrayField(TEXT("keyFrames"), KeyFrames);
  Owner->SendAutomationResponse(Socket, RequestId, true, TEXT("Paper flipbook inspected"), Result, FString());
  return true;
}
...
bool UNebulaForgeBridgeSubsystem::HandleAssetAction(
  ...
  if (Lower == TEXT("import_gltf") || Lower == TEXT("import_glb") || Lower == TEXT("import_gltf_interchange") ||
      ...
      Lower == TEXT("import_obj_interchange")) {
    FString SourcePath;
    ...
    const FString ExpectedExtension = Lower == TEXT("import_glb") ? TEXT("glb") :
        ...
        Lower == TEXT("import_obj_interchange") ? TEXT("obj") : TEXT("gltf");
  ...
  if (Lower == TEXT("inspect_tile_set_tile_metadata")) {
#if WITH_EDITOR && MCP_HAS_PAPER_TILEMAP_EDITOR
    return HandlePaperTileSetTileMetadataInspectAction(this, RequestId, Payload, RequestingSocket);
#else
    SendAutomationError(RequestingSocket, RequestId, TEXT("Paper2D editor plugin is unavailable"), TEXT("PAPER2D_EDITOR_NOT_AVAILABLE"));
    return true;
#endif
  ...
  if (Lower == TEXT("get_tile_set_uv_from_xy")) {
#if WITH_EDITOR && MCP_HAS_PAPER_TILEMAP_EDITOR
    return HandlePaperTileSetUVFromXYAction(this, RequestId, Payload, RequestingSocket);
#else
    SendAutomationError(RequestingSocket, RequestId, TEXT("Paper2D editor plugin is unavailable"), TEXT("PAPER2D_EDITOR_NOT_AVAILABLE"));
    return true;
#endif
