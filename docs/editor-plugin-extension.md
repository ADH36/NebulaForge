# NebulaForge Bridge Plugin

The NebulaForge Bridge is a production-ready Unreal Editor plugin that enables direct communication between the MCP server and Unreal Engine, providing **100% native C++ implementations** for all automation tasks. This document describes the current implementation and future roadmap.
...
## Architecture Sketch
- Editor plugin registers a `UNebulaForgeBridge` subsystem.
...
### Core Components
- **Plugin Location**: `plugins/NebulaForgeBridge/` (source and distribution-ready plugin folder)
- **Module Type**: Editor-only subsystem (`UEditorSubsystem`)
- **Main Class**: `UNebulaForgeBridgeSubsystem` - manages WebSocket connections, request routing, and automation execution
- **WebSocket Implementation**: `FMcpBridgeWebSocket` - custom lightweight WebSocket client (no external dependencies)
- **Settings**: `UNebulaForgeBridgeSettings` - configurable via **Project Settings ▸ Plugins ▸ NebulaForge Bridge**
...
### Plugin Installation
1. Copy `plugins/NebulaForgeBridge/` to your project's `Plugins/` directory, or unpack a packaged `NebulaForgeBridge/` build there
2. Regenerate project files
3. Enable plugin via **Edit ▸ Plugins ▸ NebulaForge Bridge**
4. Restart editor

### Configuration (Project Settings ▸ Plugins ▸ NebulaForge Bridge)
...
- Add handler functions to appropriate `NebulaForgeBridge_*Handlers.cpp` files
- Register new handlers in `ProcessAutomationRequest()`
- Update `NebulaForgeBridgeSubsystem.h` with handler declarations
