...
|-- plugins/NebulaForgeBridge/ # Unreal editor automation bridge + native MCP
...
| Add Unreal bridge behavior | `plugins/NebulaForgeBridge/.../Private/NebulaForgeBridge_*Handlers.cpp` | Register in `UNebulaForgeBridgeSubsystem::InitializeHandlers()` |
| Add native MCP schema/tool metadata | `plugins/NebulaForgeBridge/.../Private/MCP/` | Self-register with `MCP_REGISTER_TOOL`; keep canonical names only |
| Fix UE save/load crashes | `McpSafeOperations.h`, `NebulaForgeBridgeHelpers.h` | Use project safe wrappers and path guards |
...
| `UNebulaForgeBridgeSubsystem` | C++ class | `plugins/NebulaForgeBridge/.../Public/NebulaForgeBridgeSubsystem.h` | high | Plugin request queue, native MCP startup, handler map |
| `FMcpNativeTransport` | C++ class | `plugins/NebulaForgeBridge/.../Private/MCP/McpNativeTransport.*` | medium | Native `/mcp` HTTP/SSE JSON-RPC endpoint |
...
- Server version sources: `package.json`, `package-lock.json`, `server.json`, `src/index.ts` fallback. Plugin version source: `plugins/NebulaForgeBridge/NebulaForgeBridge.uplugin`.
