...
This project uses consolidated integration test suites covering the 28 canonical MCP tools:
...
```bash
# Ensure Unreal Engine is running with NebulaForge Bridge plugin enabled
npm test
...
Covers cross-tool workflows across the 28 canonical MCP tools:
...
2. **NebulaForge Bridge plugin** enabled and listening on port 8091
...
- NebulaForge Bridge plugin is not enabled, or
