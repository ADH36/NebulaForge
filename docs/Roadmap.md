...
- [x] `configure_aim_assist` (persists validated 0–1 aim-assist strength metadata as standard actor tags)

### 35.2 Checkpoint System
- [x] `create_checkpoint_actor` (named façade over native actor spawning; defaults to an `AActor` checkpoint anchor and `CheckpointActor` label while accepting explicit class and transform fields)
...
- [x] `get_current_reverb_effect` (API-derived audio-environment inspection using Epic's `UGameplayStatics::GetCurrentReverbEffect`, returning the active object path when present)
- [x] `get_player_controller`, `get_player_pawn` (API-derived local-player object lookup using Epic's `UGameplayStatics` player accessors, returning resolved object/class paths)
- [x] `get_player_character`, `get_player_camera_manager`, `get_player_state` (API-derived local-player object lookup using Epic's `UGameplayStatics` accessors, returning resolved object/class paths)
...
- [x] `get_closest_listener_location` (API-derived spatial-audio query using Epic's `UGameplayStatics::GetClosestListenerLocation`, including attenuation override and resolved listener position)
- [x] `get_game_instance`, `get_game_mode`, `get_game_state` (API-derived world-object lookup using Epic's `UGameplayStatics` accessors, returning resolved object/class paths and preserving null results)
- [x] `get_num_player_controllers`, `get_num_player_states` (API-derived player-count inspection using Epic's `UGameplayStatics` accessors, including remote/server semantics)
...
- [x] `create_ping_system` (named façade that spawns a transient ping actor with optional `UWidgetComponent` presentation and a bounded lifetime using Epic's `UWorld::SpawnActor` and `FTimerManager::SetTimer` APIs; `lifetimeSeconds` is validated to 0–3600 seconds)
...
- [x] `configure_photo_mode_poses` (named façade over validated actor transform authoring for camera pose location/rotation/scale, using Unreal actor transform primitives)
...
- [x] `set_skin_tone` (project-aware reflected-property façade; defaults to `SkinTone` and accepts an overrideable `propertyName` plus JSON color/value)
- [x] `set_eye_color` (project-aware reflected-property façade; defaults to `EyeColor` and accepts an overrideable `propertyName` plus JSON color/value)
...
- [x] `configure_nanite_import` (asset-scoped alias over the official Nanite rebuild/configuration path; accepts `assetPath`, `enableNanite`, and rebuild options)
- [x] `set_lod_generation` (explicit asset-tool alias over the official deferred `generate_lods` path; accepts `assetPath` and `lodCount`)
