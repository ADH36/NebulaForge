...
- `system_control.execute_python_script`, `execute_python_string`, and `execute_python_file` are explicit aliases over the secured Python execution path, matching Epic's Editor Python distinction between literal code and script files; file/code mode is validated before execution. File mode is confined to project-relative `.py` files, rejects traversal/symlink escapes, missing files, and files over the 1 MB execution limit.
