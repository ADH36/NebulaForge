...
{
public:
    FMcpRequestErrorDevice(UNebulaForgeBridgeSubsystem* InSubsystem)
...
private:
    UNebulaForgeBridgeSubsystem* Subsystem = nullptr;
};
...
DEFINE_LOG_CATEGORY(LogNebulaForgeBridgeSubsystem);
 ...
 * initialization.
 */
void UNebulaForgeBridgeSubsystem::Initialize(
  ...
  if (IsRunningCommandlet()) {
    UE_LOG(LogNebulaForgeBridgeSubsystem, Log,
           TEXT("NebulaForgeBridgeSubsystem skipping initialization - running "
                "as commandlet (cook/package mode)."));
    return;
  }

  UE_LOG(LogNebulaForgeBridgeSubsystem, Log,
         TEXT("NebulaForgeBridgeSubsystem initializing."));

  // Create and initialize the connection manager
  ConnectionManager = MakeShared<FMcpConnectionManager>();
  ConnectionManager->Initialize(GetDefault<UNebulaForgeBridgeSettings>());
  ...
  {
    const auto* Settings = GetDefault<UNebulaForgeBridgeSettings>();
    if (Settings && Settings->bEnableNativeMCP)
    {
      // Find plugin directory for loading tool schemas
      FString PluginDir;
      TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("NebulaForgeBridge"));
      if (Plugin.IsValid())
      {
  ...
  TickHandle = FTSTicker::GetCoreTicker().AddTicker(
      FTickerDelegate::CreateUObject(this,
                                     &UNebulaForgeBridgeSubsystem::Tick),
      0.0f
  );

  UE_LOG(LogNebulaForgeBridgeSubsystem, Log,
         TEXT("NebulaForgeBridgeSubsystem Initialized."));
}

/**
 * @brief Shuts down the NebulaForge Bridge subsystem and releases its
 * resources.
 *
...
void UNebulaForgeBridgeSubsystem::Deinitialize() {
  ...
  if (!IsRunningCommandlet()) {
    UE_LOG(LogNebulaForgeBridgeSubsystem, Log,
           TEXT("NebulaForgeBridgeSubsystem deinitializing."));
  }
 ...
 */
bool UNebulaForgeBridgeSubsystem::IsBridgeActive() const {
 ...
 *
 * @return ENebulaForgeBridgeState The current connection state.
 */
ENebulaForgeBridgeState
UNebulaForgeBridgeSubsystem::GetBridgeState() const {
  if (ConnectionManager.IsValid()) {
    if (ConnectionManager->GetActiveSocketCount() > 0) {
      return ENebulaForgeBridgeState::Connected;
    }
    if (ConnectionManager->IsReconnectPending()) {
      return ENebulaForgeBridgeState::Connecting;
    }
  }
  return ENebulaForgeBridgeState::Disconnected;
}
...
UNebulaForgeBridgeSubsystem::FRequestErrorCapture& UNebulaForgeBridgeSubsystem::GetCurrentErrorCapture()
{
    return CurrentErrorCapture;
}

void UNebulaForgeBridgeSubsystem::BeginErrorCapture()
{
...
TArray<FString> UNebulaForgeBridgeSubsystem::EndErrorCapture()
{
...
bool UNebulaForgeBridgeSubsystem::HasCapturedErrors() const
{
    FScopeLock Lock(&ErrorCaptureMutex);
    return CurrentErrorCapture.bHasErrors.load();
}

TArray<FString> UNebulaForgeBridgeSubsystem::GetCapturedErrorMessages() const
{
    FScopeLock Lock(&ErrorCaptureMutex);
 ...
 * `false` otherwise.
 */
bool UNebulaForgeBridgeSubsystem::SendRawMessage(const FString &Message) {
  if (ConnectionManager.IsValid()) {
 ...
 */
bool UNebulaForgeBridgeSubsystem::Tick(float DeltaTime) {
...
void UNebulaForgeBridgeSubsystem::QueueAutomationRequest(
...
// NebulaForgeBridge_ProcessRequest.cpp to avoid duplicate definitions and
...
void UNebulaForgeBridgeSubsystem::SendAutomationResponse(
 ...
 * is used if empty.
 */
void UNebulaForgeBridgeSubsystem::SendAutomationError(
    TSharedPtr<FMcpBridgeWebSocket> TargetSocket, const FString &RequestId,
 ...
 */
void UNebulaForgeBridgeSubsystem::SendProgressUpdate(
 ...
 */
void UNebulaForgeBridgeSubsystem::RecordAutomationTelemetry(
 ...
 */
void UNebulaForgeBridgeSubsystem::RegisterHandler(
 ...
 * handler.
 */
void UNebulaForgeBridgeSubsystem::InitializeHandlers() {
  // Core & Properties
 ...
 */
void UNebulaForgeBridgeSubsystem::ProcessPendingAutomationRequests() {
  if (!IsInGameThread()) {
    AsyncTask(ENamedThreads::GameThread,
 ...
 */
bool UNebulaForgeBridgeSubsystem::ExecuteEditorCommands(
...
// The MCP_HAS_CONTROLRIG_FACTORY macro is defined in NebulaForgeBridgeHelpers.h
 ...
 */
UBlueprint *UNebulaForgeBridgeSubsystem::CreateControlRigBlueprint(
  ...
  if (UEditorAssetLibrary::DoesAssetExist(FullObjectPath)) {
    ...
    if (ExistingAsset) {
      ...
      if (ExistingAsset->IsA<UControlRigBlueprint>()) {
        ...
        UE_LOG(LogNebulaForgeBridgeSubsystem, Error, TEXT("%s"),
               *OutError);
        return nullptr;
      }
    }
