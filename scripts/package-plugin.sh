#!/usr/bin/env bash
#
# Package NebulaForgeBridge plugin as pre-built binaries.
...
PLUGIN_FILE="$REPO_ROOT/plugins/NebulaForgeBridge/NebulaForgeBridge.uplugin"
...
ZIP_NAME="NebulaForgeBridge-v${PLUGIN_VER}-UE${UE_VER}-${PLATFORM}.zip"
ZIP_PATH="$OUTPUT_DIR/$ZIP_NAME"
STAGING_DIR="$(mktemp -d "${TMPDIR:-/tmp}/NebulaForgeBridge-package.XXXXXX")"
PACKAGE_DIR="$STAGING_DIR/NebulaForgeBridge"
...
if [ -f "$PACKAGE_DIR/NebulaForgeBridge.uplugin" ]; then
    OUTPUT_PLUGIN_DIR="$PACKAGE_DIR"
elif [ -f "$PACKAGE_DIR/HostProject/Plugins/NebulaForgeBridge/NebulaForgeBridge.uplugin" ]; then
    OUTPUT_PLUGIN_DIR="$PACKAGE_DIR/HostProject/Plugins/NebulaForgeBridge"
else
    echo "ERROR: Packaged plugin output not found under: $PACKAGE_DIR"
    exit 1
fi

OUTPUT_UPLUGIN="$OUTPUT_PLUGIN_DIR/NebulaForgeBridge.uplugin"
...
zip -r "$ZIP_PATH" NebulaForgeBridge/ \
    -x "*.pdb" \
    -x "NebulaForgeBridge/Intermediate/*"
echo ""
