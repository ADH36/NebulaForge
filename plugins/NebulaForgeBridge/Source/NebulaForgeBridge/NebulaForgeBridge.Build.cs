...
public class NebulaForgeBridge : ModuleRules
{
    ...
    /// <summary>
    /// Configures build rules, dependencies, and compile-time feature definitions for the NebulaForgeBridge module based on the provided build target.
    /// </summary>
    ...
    public NebulaForgeBridge(ReadOnlyTargetRules Target) : base(Target)
    {
        // ============================================================================
        ...
        if (Target.Version.MajorVersion == 5 && Target.Version.MinorVersion <= 2)
        {
            if (Target.Platform == UnrealTargetPlatform.Win64)
            {
                try
                {
                    ...
                    if (innerField != null)
                    {
                        ...
                        if (targetRules != null)
                        {
                            ...
                            {
                                Console.WriteLine("NebulaForgeBridge: Disabled bUndefinedIdentifierErrors for UE 5.0-5.2 MSVC build");
                            }
                            ...
                            if (!targetRules.GlobalDefinitions.Contains(HasFeatureDefine))
                            {
                                ...
                                Console.WriteLine("NebulaForgeBridge: Added __has_feature(x)=0 to GlobalDefinitions");
                            }
                        }
                    }
                }
                ...
                {
                    Console.WriteLine(string.Format("NebulaForgeBridge: WARNING: Could not disable bUndefinedIdentifierErrors for UE 5.{0}: {1}", Target.Version.MinorVersion, Ex.Message));
                }
                Console.WriteLine(string.Format("NebulaForgeBridge: Applied MSVC __has_feature compatibility for UE 5.{0}", Target.Version.MinorVersion));
            }
        }
        ...
        Console.WriteLine(string.Format("NebulaForgeBridge: Detected {0}MB available memory (of {1}MB total)", AvailableMemoryMB, TotalMemoryMB));
        ...
        if (Target.bBuildEditor)
        {
            ...
            {
                Console.WriteLine(string.Format("NebulaForgeBridge: WorldPartition support detection failed: {0}", Ex.Message));
            }
    ...
    private long GetTotalPhysicalMemoryMB()
    {
        ...
        {
            Console.WriteLine(string.Format("NebulaForgeBridge: Total memory detection failed: {0}", Ex.Message));
        }
    ...
    private bool AddOptionalDynamicModule(ReadOnlyTargetRules Target, string EngineDir, string ModuleName, string SearchName)
    {
        if (FindOptionalModule(EngineDir, SearchName))
        {
            ...
            Console.WriteLine(string.Format("NebulaForgeBridge: Added optional module '{0}' with delay-load", ModuleName));
            return true;
        }
    ...
    private bool AddOptionalConditionalModule(ReadOnlyTargetRules Target, string EngineDir, string ModuleName, string SearchName)
    {
        if (FindOptionalModule(EngineDir, SearchName))
        {
            ...
            PrivateDependencyModuleNames.Add(ModuleName);
            Console.WriteLine(string.Format("NebulaForgeBridge: Added optional module '{0}' (conditional)", ModuleName));
            return true;
        }
