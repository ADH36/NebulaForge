...
// McpHandlerUtils.h
// =============================================================================
// Centralized utility functions and macros for NebulaForge Bridge handlers.
//
// This file provides:
...
//
// REFACTORING NOTES:
// - Functions extracted from NebulaForgeBridge_BlueprintHandlers.cpp (900+ lines)
...
class UNebulaForgeBridgeSubsystem;
...
namespace McpHandlerUtils
{
     ...
     */
    NEBULAFORGEBRIDGE_API FString JsonValueToString(const TSharedPtr<FJsonValue>& Value);
     ...
     */
    NEBULAFORGEBRIDGE_API FString ValidateAssetPath(const FString& Path);
     ...
     */
    NEBULAFORGEBRIDGE_API class AActor* FindActorByName(const FString& ActorName, bool bExactMatch = true);
     ...
     */
    NEBULAFORGEBRIDGE_API class UActorComponent* FindActorComponentByName(
        class AActor* Actor,
     ...
     */
    NEBULAFORGEBRIDGE_API FString ToSafeAssetName(const FString& Input);
     ...
     */
    NEBULAFORGEBRIDGE_API FString MakeUniqueAssetName(const FString& BaseName, const FString& PackagePath);
     ...
     */
    NEBULAFORGEBRIDGE_API UObject* ResolveObjectFromPath(
     ...
     */
    NEBULAFORGEBRIDGE_API FPropertyResolveResult ResolveProperty(
        UObject* Object,
     ...
     */
    NEBULAFORGEBRIDGE_API void AddVerification(TSharedPtr<FJsonObject>& Result, UObject* Object);
}
...
namespace McpBlueprintUtils
{
     ...
     */
    NEBULAFORGEBRIDGE_API UEdGraphPin* FindExecPin(UEdGraphNode* Node, EEdGraphPinDirection Direction);

    /**
     * Find an output pin on a node by name.
     */
    NEBULAFORGEBRIDGE_API UEdGraphPin* FindOutputPin(UEdGraphNode* Node, const FName& PinName = NAME_None);

    /**
     * Find an input pin on a node by name.
     */
    NEBULAFORGEBRIDGE_API UEdGraphPin* FindInputPin(UEdGraphNode* Node, const FName& PinName);

    /**
     * Find a data pin on a node by direction and optionally by name.
     */
    NEBULAFORGEBRIDGE_API UEdGraphPin* FindDataPin(UEdGraphNode* Node, EEdGraphPinDirection Direction, const FName& PreferredName = NAME_None);

    /**
     * Find the preferred event execution output pin in a graph.
     * Prefers custom events, falls back to first available event.
     */
    NEBULAFORGEBRIDGE_API UEdGraphPin* FindPreferredEventExec(UEdGraph* Graph);
     ...
     */
    NEBULAFORGEBRIDGE_API FEdGraphPinType MakePinType(const FString& TypeName);

    /**
     * Convert an FEdGraphPinType to a human-readable string.
     * Handles containers (Array, Set, Map) and complex types.
     */
    NEBULAFORGEBRIDGE_API FString DescribePinType(const FEdGraphPinType& PinType);
     ...
     */
    NEBULAFORGEBRIDGE_API UK2Node_VariableGet* CreateVariableGetter(UEdGraph* Graph, const FMemberReference& VarRef, float NodePosX, float NodePosY);

    /**
     * Log a pin connection failure for debugging.
     */
    NEBULAFORGEBRIDGE_API void LogConnectionFailure(const TCHAR* Context, UEdGraphPin* SourcePin, UEdGraphPin* TargetPin, const FPinConnectionResponse& Response);
     ...
     */
    NEBULAFORGEBRIDGE_API TArray<TSharedPtr<FJsonValue>> CollectBlueprintVariables(UBlueprint* Blueprint);

    /**
     * Collect all functions from a blueprint as JSON.
     */
    NEBULAFORGEBRIDGE_API TArray<TSharedPtr<FJsonValue>> CollectBlueprintFunctions(UBlueprint* Blueprint);

    /**
     * Find a property in a blueprint by name.
     */
    NEBULAFORGEBRIDGE_API FProperty* FindBlueprintProperty(UBlueprint* Blueprint, const FString& PropertyName);

    /**
     * Find a function in a blueprint by name.
     */
    NEBULAFORGEBRIDGE_API UFunction* FindBlueprintFunction(UBlueprint* Blueprint, const FString& FunctionName);
}
