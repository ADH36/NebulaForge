...
Packaged-runtime update (2026-08-31): the plugin now includes an opt-in `NebulaForgeBridgeRuntime` Runtime module. When enabled with `[NebulaForgeBridgeRuntime.NebulaForgeRuntimeSettings] bEnabled=true` in the game config or `NEBULAFORGE_RUNTIME_BRIDGE_ENABLED=true`, it exposes a loopback-only WebSocket bridge on port `8092` (configurable) with optional `NEBULAFORGE_RUNTIME_CAPABILITY_TOKEN` authentication. The bridge bounds concurrent clients to eight, expires incomplete WebSocket or application handshakes after five seconds, and closes authenticated clients after five minutes of inactivity. Its runtime-safe contract includes health/capability/world inspection plus bounded actor listing and exact-name actor inspection (`get_runtime_actors`, `get_runtime_actor`); it does not provide packaged asset authoring, editor commands, filesystem access, or arbitrary console execution. This gives AI a supported runtime inspection/proof channel without weakening the editor/runtime security boundary.
...
| Save/load/delete slots | ⚠️ | Editor and packaged/runtime slot save/load/delete/existence actions are implemented for `USaveGame` objects, including managed async save/load lifecycle IDs, bounded deadlines, exactly-once terminal events, and `timed_out` polling state. Runtime `list_save_game_slots` now enumerates local `Saved/SaveGames/*.sav` records with size and user-index metadata; cloud/provider slot enumeration remains unavailable through a universal UE API. Higher-level slot serialization orchestration remains a project integration boundary. |
...
| Generic asset tagging/metadata | ✅ | Generic tag and package-metadata set/get/find actions are implemented with normalized asset paths. Reflected data-asset properties and data-table row CRUD are also available through `manage_asset`. |
...
| Packaged plugin loading | ⚠️ | The editor bridge remains `Type: Editor`, but packaged targets now load the opt-in `NebulaForgeBridgeRuntime` module. Runtime bridge startup is disabled by default and only exposes three runtime-safe inspection actions. |
...
| Packaged/runtime inspection | ⚠️ | The opt-in runtime bridge provides health, capability, current-world, and bounded actor inspection in the packaged process; gameplay-specific probes, input delivery, and runtime authoring remain project-specific or unsupported. |
| Standalone PIE probes/input | ❌ | In-process probes and input delivery are not supported because standalone runs in another process; the packaged runtime bridge is limited to its explicit inspection contract. |
...
| Unified asynchronous job system | ⚠️ | Host processes, bounded filesystem tasks, waited PCG generation, HLOD commandlets, shader compilation, and material compilation expose managed lifecycle IDs; host jobs provide output caps, while native jobs provide cancellation, timeout, completion events, and polling. Other editor delegates still use request-scoped guards. |
| Completion proof | ⚠️ | Host jobs expose `wait_for_job`, and native managed async actions expose `wait_for_async_action` with timeout/cancel/success state reporting; PCG, HLOD, shader, and material jobs feed that registry. Remaining editor delegates and packaged-runtime work still require separate integration. |
...
The editor WebSocket handshake advertises `executionMode: editor`, `pieRuntimeWorld: true`, and `packagedRuntimeAuthoring: false`. The packaged bridge uses a separate Runtime module and advertises `executionMode: runtime` plus its allowlisted inspection actions. Packaged runtime asset authoring still remains intentionally unsupported; changing `WITH_EDITOR` guards alone would be unsafe.
...
UE 5.8 packaging verification: the current bridge source compiled successfully through all 149 UnrealBuildTool actions, including both `UnrealEditor` and `UnrealGame` targets, and produced `NebulaForgeBridge-v0.5.30-UE5.8-Win64.zip` with the short-path package workflow on 2026-08-31. The packaged artifact includes the opt-in runtime module. This validates native compilation and packaging only; it does not replace live project, editor, PIE, packaged-game behavior, or platform-service verification.
