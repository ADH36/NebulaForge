...
## Architecture
- This is a dual-process system: the TypeScript MCP server lives in `src/`, and the Unreal Editor bridge plugin lives in `plugins/NebulaForgeBridge/`.
...
- Keep workspace-wide guidance here and rely on the closer `AGENTS.md` files for area-specific detail under `src/tools/`, `src/tools/handlers/`, `src/automation/`, `tests/`, and `plugins/NebulaForgeBridge/`.
...
## UE 5.7 safety
- Do not use `UPackage::SavePackage()` in plugin code. Use the safe helper wrappers in `plugins/NebulaForgeBridge/Source/NebulaForgeBridge/Public/NebulaForgeBridgeHelpers.h`.
...
- New MCP action flow:
	...
	3. Implement the Unreal side in the appropriate handler under `plugins/NebulaForgeBridge/Source/` and register it in `UNebulaForgeBridgeSubsystem::InitializeHandlers()`.
...
- If you change versions, update all version-bearing files together, including `package.json`, `server.json`, `src/index.ts`, and `plugins/NebulaForgeBridge/NebulaForgeBridge.uplugin`.
...
- Use `plugins/NebulaForgeBridge/AGENTS.md` for plugin-side patterns and Unreal-specific caveats.
