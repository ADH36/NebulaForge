...
- [`UMoviePipelineQueue`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/MovieRenderPipelineCore/UMoviePipelineQueue) and [`UMoviePipelineExecutorJob`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/MovieRenderPipelineCore/UMoviePipelineExecutorJob) provide the UE 5 Movie Render Queue job/queue model used by `manage_sequence.render_sequence_mrq`.
- [`UMediaPlayer`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/MediaAssets/UMediaPlayer) and Epic's [Media Framework overview](https://dev.epicgames.com/documentation/unreal-engine/media-framework-overview-for-unreal-engine) define the player/source/texture/playlist playback workflow exposed by `manage_asset` and `control_editor`.
- [`UMediaSoundComponent`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/MediaAssets/UMediaSoundComponent) and [`UMediaSoundComponent::SetMediaPlayer`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/MediaAssets/UMediaSoundComponent/SetMediaPlayer) back `control_actor.create_media_sound_component`, attaching a player's audio output to an actor.
- [`UCineCameraComponent`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/CinematicCamera/UCineCameraComponent) exposes `Filmback`, `FocusSettings`, `CurrentFocalLength`, and `CurrentAperture`; these are the reflected properties targeted by `control_actor.configure_camera_settings`.
- [`UMovieSceneCameraShakeTrack`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/MovieSceneTracks/UMovieSceneCameraShakeTrack) and [`UMovieSceneCameraShakeTrack::AddNewCameraShake`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/MovieSceneTracks/UMovieSceneCameraShakeTrack/AddNewCameraShake) back `manage_sequence.add_camera_shake_track`, which resolves a `UCameraShakeBase` subclass and keys it at the requested frame.
- [`UTakeRecorderSubsystem`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/TakeRecorder/UTakeRecorderSubsystem) provides the optional `StartRecording`/`StopRecording` lifecycle used by `control_editor.start_take_recording` and `stop_take_recording`; Epic's [Take Recorder overview](https://dev.epicgames.com/documentation/unreal-engine/take-recorder-in-unreal-engine) documents its Sequencer integration and source model.
- [`ACameraRig_Rail`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/CinematicCamera/ACameraRig_Rail) exposes normalized rail position and orientation-lock settings, while [`ACameraRig_Crane`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/CinematicCamera/ACameraRig_Crane) exposes arm length, pitch, yaw, and mount locks; these are validated by `control_actor.configure_camera_rig_rail` and `configure_camera_rig_crane`.
- [`UMoviePipelineAntiAliasingSetting`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/MovieRenderPipelineCore/UMoviePipelineAntiAliasingSettin-) exposes `SpatialSampleCount` and `TemporalSampleCount`; `manage_sequence.render_sequence_mrq` applies these to the transient job when requested.
- [`UDemoNetDriver`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UDemoNetDriver) and Epic's [Replay System guide](https://dev.epicgames.com/documentation/unreal-engine/using-the-replay-system-in-unreal-engine) define replay playback, scrubbing, pausing, and time dilation; these are exposed through `control_editor.play_demo`, `pause_demo`, `seek_demo`, and `set_demo_playback_speed`.
- The existing editor recording bridge invokes Unreal's `DemoRec`/`DemoStop` console path; `control_editor.start_demo_recording` and `stop_demo_recording` now expose explicit replay-recording aliases alongside the existing generic recording actions.
- [`UMoviePipelineObjectIdRenderPass`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/MoviePipelineMaskRenderPass/UMoviePipelineObjectIdRenderPass) is the optional UE 5 Additional Render Passes setting used by `manage_sequence.render_sequence_mrq` with `renderPass: "object_id"`; Epic's [Cinematic Render Passes guide](https://dev.epicgames.com/documentation/unreal-engine/cinematic-render-passes-in-unreal-engine) documents its editor-only/plugin and EXR prerequisites.
- [`UDataAsset`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/Engine/UDataAsset) and [`UDataTable::AddRow`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UDataTable/AddRow) back the reflected data-asset and data-table authoring actions in `manage_asset`; [`UDataTable::FindRow`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UDataTable/FindRow) is used by row read/modify validation.
- [`UDataTable::CreateTableFromCSVString`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UDataTable/CreateTableFromCSVString) and [`UDataTable`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UDataTable) `GetTableAsCSV` back `manage_asset.import_data_table_csv` and `export_data_table_csv`.
- [`UPrimaryDataAsset`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UPrimaryDataAsset) implements `GetPrimaryAssetId` and asset-bundle support; `manage_asset.create_primary_data_asset` validates and instantiates concrete subclasses so the abstract base is never created directly.
- [`UGameplayStatics::SaveGameToSlot`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UGameplayStatics/SaveGameToSlot), Epic's [Save/Load guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/saving-and-loading-your-game-in-unreal-engine), and the native async save API support the `system_control` SaveGame slot lifecycle and managed asynchronous operations.
- [`FGameplayTagContainer`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/GameplayTags/FGameplayTagContainer) supplies `AddTag`, `RemoveTag`, `HasAny`, `HasAll`, and exact-match variants used by the transient `system_control` tag-container actions; Epic's [Gameplay Tags guide](https://dev.epicgames.com/documentation/unreal-engine/using-gameplay-tags-in-unreal-engine) documents the registered-tag requirement.
- [`UGameplayTagsManager::AddNativeGameplayTag`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/GameplayTags/UGameplayTagsManager/AddNativeGameplayTag) backs `system_control.register_native_tag`; Epic notes that late registration can invalidate fast replication, so the action reports the tag as process-only and does not edit project config.
- [`FGameplayTagTableRow`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/GameplayTags/FGameplayTagTableRow), [`UDataTable`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UDataTable), and Epic's [Gameplay Tags guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/using-gameplay-tags-in-unreal-engine) back `manage_asset.create_tag_table`, which creates a persisted DataTable with the Gameplay Tag row schema.
- [`RecompileShaders`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/RecompileShaders) and Epic's [shader compilation guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/adding-global-shaders-to-unreal-engine) back `system_control.compile_shaders`, which exposes the engine's `recompileshaders changed|all` command.
- [`UDeviceProfileManager`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/DeviceProfiles/UDeviceProfileManager) and Epic's [device profile guide](https://dev.epicgames.com/documentation/unreal-engine/setting-up-device-profiles-in-unreal-engine) back `system_control.create_device_profile`, which safely authors a project `DefaultDeviceProfiles.ini` profile section.
- The same Epic device-profile configuration format backs `system_control.set_cvar_for_profile`, which updates a single `+CVars` override in an existing profile section.
- Epic's configuration and platform-profile guidance backs `system_control.configure_build_settings` and `configure_platform_settings`; both use the repository's guarded project `Config/*.ini` writer for explicit section/key/value edits.
- Epic's BuildCookRun packaging flow backs `system_control.create_pak_file`, `configure_compression`, and `configure_asset_encryption`; these aliases select package mode and validated `-compressed`, `-encryptinifiles`, and `-encryptpakindex` options.
- Epic's [Functional Testing guide](https://dev.epicgames.com/documentation/unreal-engine/functional-testing-in-unreal-engine) backs `system_control.create_test_level`, which delegates to the existing safe UE level authoring path so functional tests can be placed in a dedicated map.
- Epic's [Running Gauntlet Tests guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/running-gauntlet-tests) backs `system_control.run_gauntlet_test`, which launches the documented `RunUAT RunUnreal` command through the managed job runner.
- Epic's [Replay System guide](https://dev.epicgames.com/documentation/unreal-engine/using-the-replay-system-in-unreal-engine), [UDemoNetDriver API](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UDemoNetDriver), and [console variable reference](https://dev.epicgames.com/documentation/en-us/unreal-engine/unreal-engine-console-variables-reference) back `system_control.configure_demo_settings`, restricted to documented `demo.*` keys.
- Epic's [`UTakeRecorderSubsystem`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/TakeRecorder/UTakeRecorderSubsystem) and [`ITakeRecorderSubsystemInterface`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/TakeRecorder/ITakeRecorderSubsystemInterface) back `control_editor.configure_take_sources`, which adds resolved actors with the official `AddSourceForActor` API.
- Epic's [Replay System guide](https://dev.epicgames.com/documentation/unreal-engine/using-the-replay-system-in-unreal-engine) and [`UDemoNetDriver`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UDemoNetDriver) back `control_editor.configure_killcam_duration` and `start_killcam`; the bridge seeks to an explicit replay time, starts playback, and schedules a bounded `DemoPause`.
- Epic's [`UTakeRecorderSources`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/TakesCore/UTakeRecorderSources), [`FTakeRecorderSourcesSettings`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/TakesCore/FTakeRecorderSourcesSettings), and [`UTakeRecorderSubsystem::GetSources`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/TakeRecorder/UTakeRecorderSubsystem) back `control_editor.configure_recorded_tracks`, which applies documented possessable, redundant-track, asset-save, sub-sequence, lock, and timecode policies.
- Epic's [Localization Tools guide](https://dev.epicgames.com/documentation/unreal-engine/localization-tools-in-unreal-engine), [localization overview](https://dev.epicgames.com/documentation/en-us/unreal-engine/localization-overview-for-unreal-engine), and [`UGatherTextCommandlet`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Editor/UnrealEd/UGatherTextCommandlet) back `system_control.configure_localization_target`, which writes a constrained GatherText INI under `Config/Localization`.
- The same [`UGatherTextCommandlet`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Editor/UnrealEd/UGatherTextCommandlet) and Epic's [localization pipeline guidance](https://dev.epicgames.com/documentation/unreal-engine/pipeline-optimization-for-localization-in-unreal-engine) back `system_control.import_localization` and `export_localization`, launched as managed `UnrealEditor-Cmd` jobs against project-local configs.
- Epic's [Unreal Engine Build Tool Target Reference](https://dev.epicgames.com/documentation/en-us/unreal-engine/unreal-engine-build-tool-target-reference) backs `system_control.create_build_target`, which generates a project-relative `.Target.cs` deriving from `TargetRules` with a documented target type.
- Epic's [Automation Test Framework](https://dev.epicgames.com/documentation/unreal-engine/automation-test-framework-in-unreal-engine) and [Functional Testing](https://dev.epicgames.com/documentation/unreal-engine/functional-testing-in-unreal-engine) guides back `system_control.configure_test_settings`; the action uses the guarded project config service for explicit test-related INI settings.
- Epic's [World Partition HLOD guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/world-partition---hierarchical-level-of-detail) backs `manage_level_structure.configure_hlod_transition`, using the HLOD layer's documented Cell Size and Loading Range settings.
- Epic's [Gameplay Tags guide](https://dev.epicgames.com/documentation/unreal-engine/using-gameplay-tags-in-unreal-engine) documents project tag dictionary configuration; `system_control.create_gameplay_tag` writes the corresponding `GameplayTagsSettings` entry through the safe project-file service.
- Epic's [Save/Load guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/saving-and-loading-your-game-in-unreal-engine) also defines the custom `USaveGame` class workflow; `system_control.generate_save_game_class` generates the class and declared variables used by the slot APIs.
- [`FConfigCacheIni`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/FConfigCacheIni), including [`GetString`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/Misc/FConfigCacheIni/GetString) and [`SetString`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/Misc/FConfigCacheIni/SetString), is the official UE 5 section/key configuration API; Epic's [configuration files guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/configuration-files-in-unreal-engine) describes the layered `.ini` model mirrored by the safe `system_control` config actions.
- [`IPluginManager`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Projects/IPluginManager), [`IPlugin::IsEnabled`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Projects/IPlugin/IsEnabled), and [`FPluginStatus`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Projects/FPluginStatus) define UE 5 plugin discovery and status; the dedicated project actions expose the safe descriptor-level portion and leave live module loading to Unreal startup.
- [`UDataValidationCommandlet`](https://dev.epicgames.com/documentation/unreal-engine/API/Plugins/DataValidation/UDataValidationCommandlet) and Epic's [Data Validation guide](https://dev.epicgames.com/documentation/unreal-engine/data-validation-in-unreal-engine) define the `UnrealEditor-Cmd.exe <project>.uproject -run=DataValidation` workflow exposed by `system_control.run_data_validation`.
- [`UEditorValidatorBase`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/DataValidation/UEditorValidatorBase), [`CanValidateAsset_Implementation`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/DataValidation/UEditorValidatorBase/CanValidateAsset_Implementation), and [`ValidateLoadedAsset_Implementation`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/DataValidation/UEditorValidatorBase/ValidateLoadedAsset-?lang=en-US) define the compile-ready validator skeleton generated by `system_control.create_asset_validator`.
- Epic's [Build Operations guide](https://dev.epicgames.com/documentation/unreal-engine/build-operations-cooking-packaging-deploying-and-running-projects-in-unreal-engine) and [Content Cooking guide](https://dev.epicgames.com/documentation/unreal-engine/cooking-content-in-unreal-engine) define UAT `BuildCookRun` and platform cooking; `system_control.cook_content` and `package_project` select those validated pipeline modes.
- Epic's [Build Operations guide](https://dev.epicgames.com/documentation/unreal-engine/build-operations-cooking-packaging-deploying-and-running-projects-in-unreal-engine) also documents the command-line build/cook pipeline that `system_control.run_ubt` and the managed UAT actions wrap.
- [`UHLODLayer`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UHLODLayer), [`UWorldPartition`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UWorldPartition), and Epic's [World Partition HLOD guide](https://dev.epicgames.com/documentation/unreal-engine/world-partition---hierarchical-level-of-detail-in-unreal-engine) back the `manage_level_structure` HLOD layer, assignment, and rebuild operations.
- [`FStringTableRegistry`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/FStringTableRegistry), [`FStringTable`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/FStringTable), [`FStringTable::SetSourceString`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/Internationalization/FStringTable/SetSourceString), and Epic's [String Tables guide](https://dev.epicgames.com/documentation/unreal-engine/using-string-tables-for-text-in-unreal-engine) back the transient `system_control` string-table actions.
- [`FInternationalization::SetCurrentCulture`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/Internationalization/FInternationalization/SetCurrentCulture), [`FInternationalization::GetCurrentCulture`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/FInternationalization/GetCurrentCulture), and [`FCulture::GetName`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/FCulture/GetName) back `system_control.set_culture`, which accepts an IETF language tag and returns the active culture name; Epic's [runtime culture guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/managing-the-active-culture-at-runtime) notes that setting the culture updates language and locale together.
- [`FInternationalization::SetCurrentLanguageAndLocale`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/FInternationalization/SetCurrentLanguageAndLocale) and [`FInternationalization::SetCurrentLocale`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Core/FInternationalization/SetCurrentLocale) back the unlisted `system_control.set_language_and_locale` and `set_locale` actions, returning language, locale, and effective culture readback.
- [`UMoviePipelineBurnInSetting`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/MovieRenderPipelineSettings/UMoviePipelineBurnInSetting) and [`UMoviePipelineBurnInWidget`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/MovieRenderPipelineSettings/UMoviePipelineBurnInWidget) back `manage_sequence.configure_burn_ins` through the transient MRQ job configuration; the action reports unavailable when the optional Movie Render Pipeline Settings module is not present.
- [`Scalability::FQualityLevels`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/FQualityLevels), its [`SetFromSingleQualityLevel`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/FQualityLevels/SetFromSingleQualityLevel) helper, and Epic's [Scalability Reference](https://dev.epicgames.com/documentation/unreal-engine/scalability-reference-for-unreal-engine) back `system_control.set_quality_level`, which applies the documented 0-4 overall quality scale.
- Epic's [Scalability Reference](https://dev.epicgames.com/documentation/unreal-engine/scalability-reference-for-unreal-engine) documents the `sg.*Quality` groups; `system_control.configure_scalability_group` validates a requested group and applies its corresponding scalability CVar through the existing command-validation boundary.
- Epic's [Project Files guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/how-to-generate-unreal-engine-project-files-for-your-ide) documents UBT's `-ProjectFiles` mode; `system_control.generate_project_files` invokes that mode through the managed UBT job path.
- Epic's [cooking and chunking guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/cooking-content-and-creating-chunks-in-unreal-engine) and the [`IAssetRegistry`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/AssetRegistry/IAssetRegistry) dependency APIs back `manage_asset.audit_assets`, which reports the cached asset inventory and per-asset dependency/referencer counts for a selected package path.
- Epic's [`UProjectPackagingSettings`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Developer/DeveloperToolSettings/UProjectPackagingSettings) and [chunking guide](https://dev.epicgames.com/documentation/en-us/unreal-engine/preparing-assets-for-chunking-in-unreal-engine) back `system_control.configure_chunking`, which writes the documented chunk-generation, hard-reference, single-chunk, HTTP-install, and maximum-size settings to `DefaultGame.ini`.
- Epic's [`UInstancedStaticMeshComponent`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UInstancedStaticMeshComponent) and [`UHierarchicalInstancedStaticMeshComponent`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UHierarchicalInstancedStaticMesh-) back `control_actor.create_instanced_static_mesh_component` and `create_hierarchical_instanced_static_mesh`, which create the corresponding native components through the existing actor-component authoring path.
- Epic's [`AddInstance`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UInstancedStaticMeshComponent/AddInstance), [`RemoveInstance`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UInstancedStaticMeshComponent), and [`UpdateInstanceTransform`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/Components/UInstancedStaticMeshComponent/UpdateInstanceTr-_1) back the ISM/HISM instance add, remove, and transform-update actions.
- Epic's documented ISM culling fields [`InstanceStartCullDistance`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UInstancedStaticMeshComponent) and `InstanceEndCullDistance`, plus `InstanceMinDrawDistance`, `InstanceLODDistanceScale`, and `bUseGpuLodSelection`, back `control_actor.configure_instance_culling` and `configure_instance_lod`.
- Epic's [`IAssetRegistry::GetAssets`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/AssetRegistry/IAssetRegistry/GetAssets), [`FAssetPackageData::DiskSize`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/CoreUObject/FAssetPackageData), and [`FPackageName::TryConvertLongPackageNameToFilename`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/CoreUObject/FPackageName/TryConvertLongPackageNameToFilen-) back `manage_asset.size_map_analysis`, which reports package-file sizes for the selected asset directory.
- Epic's [`UDeveloperSettings`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/DeveloperSettings/UDeveloperSettings) and [Project Settings configuration guidance](https://dev.epicgames.com/documentation/en-us/unreal-engine/project-settings-in-unreal-engine) back `system_control.configure_plugin_settings`, which safely authors a plugin's project `.ini` settings section without requiring the plugin's private C++ types.
- Epic's [Project Settings guidance](https://dev.epicgames.com/documentation/en-us/unreal-engine/project-settings-in-unreal-engine), including platform `.ini` configuration, backs `system_control.configure_windows_build`, `configure_linux_build`, and `configure_mac_build`; each writes a validated key/value to the corresponding platform target-settings section by default.
- Epic's [Project Settings guidance](https://dev.epicgames.com/documentation/en-us/unreal-engine/project-settings-in-unreal-engine) and [DeveloperSettings API](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/DeveloperSettings/UDeveloperSettings) back `system_control.configure_ios_build` and `configure_android_build`; signing actions remain gated on the platform SDK and credentials.
- Epic's [Reference Viewer guide](https://dev.epicgames.com/documentation/unreal-engine/reference-viewer-in-unreal-engine), [`IAssetRegistry::GetDependencies`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/AssetRegistry/AssetRegistry/IAssetRegistry/GetDependencies), and [`IAssetRegistry::GetReferencers`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/AssetRegistry/IAssetRegistry/GetReferencers) back `manage_asset.reference_viewer`; the action returns forward and reverse `/Game` package graphs with bounded depth.

Epic's [Niagara scalability and pooling guidance](https://dev.epicgames.com/documentation/en-us/unreal-engine/scalability-and-best-practices-for-niagara), [`UNiagaraComponent`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Niagara/UNiagaraComponent), and [`UActorComponent::Deactivate`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/Components/UActorComponent/Deactivate) back `manage_effect.create_effect_pool`, `spawn_pooled_effect`, `release_pooled_effect`, and `destroy_effect_pool`; the bridge prewarms reusable Niagara actors and makes ownership policy explicit.
- Epic's [`UProceduralMeshComponent`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/UProceduralMeshComponent) backs `control_actor.create_procedural_mesh_component`; the action uses the existing safe component authoring path and reports the optional plugin requirement when the class is unavailable.
- Epic's [`CreateMeshSection`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/UProceduralMeshComponent/CreateMeshSection), [`UpdateMeshSection`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/UProceduralMeshComponent/UpdateMeshSection), and section-clear APIs back `manage_geometry.create_mesh_section`, `update_mesh_section`, `clear_mesh_section`, and `clear_all_mesh_sections`. The bridge validates vertex/index bounds before calling the component APIs and supports optional normals and UV0 data.
- Epic's [`AddCollisionConvexMesh`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/UProceduralMeshComponent/AddCollisionConvexMesh), [`ClearCollisionConvexMeshes`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/UProceduralMeshComponent/ClearCollisionConvexMeshes), and `bUseComplexAsSimpleCollision` APIs back `manage_geometry.add_collision_convex_mesh`, `clear_collision_convex_meshes`, and `set_collision_from_mesh`. Convex input is validated to contain at least four points, while collision enablement is mapped to Unreal's query-and-physics or no-collision modes.
- Epic's [`SetMeshSectionVisible`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/UProceduralMeshComponent/SetMeshSectionVisible) and [`IsMeshSectionVisible`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/UProceduralMeshComponent/IsMeshSectionVisible) APIs are exposed as `manage_geometry.set_mesh_section_visibility` and `get_mesh_section_visibility`.
- `manage_geometry.set_mesh_section_material` uses Unreal's `UMeshComponent::SetMaterial` on the requested section and validates the project material path before assignment.
- `control_actor.spawn_paper_sprite_actor` and `spawn_paper_flipbook_actor` use Epic's [`APaperSpriteActor`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/APaperSpriteActor), [`UPaperSpriteComponent::SetSprite`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSpriteComponent/SetSprite), [`APaperFlipbookActor`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/APaperFlipbookActor), and [`UPaperFlipbookComponent::SetFlipbook`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperFlipbookComponent/SetFlipbook). The actions are guarded by Paper2D header availability and require a pre-existing asset path.
- `control_actor.spawn_paper_tile_map_actor` uses Epic's [`APaperTileMapActor`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/APaperTileMapActor), [`APaperTileMapActor::GetRenderComponent`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2DAPaperTileMapActor/GetRenderComponent), and [`UPaperTileMapComponent::SetTileMap`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMapComponent/SetTileMap) to place an existing tile-map asset in the level; tile painting and tile-set authoring remain separate roadmap items.
- `control_actor.set_paper_tile_map_color` uses Epic's [`UPaperTileMapComponent::SetTileMapColor`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMapComponent/SetTileMapColor) to apply the tile-map-wide vertex color multiplier to an existing component.
- `control_actor.set_paper_tile_map_layer_color` uses Epic's [`UPaperTileMapComponent::SetLayerColor`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMapComponent/SetLayerColor) to apply a per-layer vertex color multiplier; the layer index is validated as non-negative.
- `control_actor.get_paper_tile_map_layer_color` uses Epic's [`UPaperTileMapComponent::GetLayerColor`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMapComponent/GetLayerColor) to read a validated layer's color multiplier.
- `control_actor.get_paper_tile_map_info` uses Epic's [`UPaperTileMapComponent::GetMapSize`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMapComponent/GetMapSize) and [`GetRenderingStats`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMapComponent/GetRenderingStats) to report dimensions, layer count, triangles, and render batches without modifying the asset.
- `control_actor.get_paper_tile_map_parameters` uses Epic's [`UPaperTileMap::GetTileToLocalParameters`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMap/GetTileToLocalParameters) and [`GetLocalToTileParameters`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMap/GetLocalToTileParameters) to expose both directions of the tile-grid conversion basis.
- `control_actor.get_paper_tile` uses Epic's [`UPaperTileMapComponent::GetTile`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMapComponent/GetTile), [`GetTileCenterPosition`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMapComponent/GetTileCenterPosition), and [`GetTileCornerPosition`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMapComponent/GetTileCornerPosition) to read a packed tile reference and its local or world-space positions.
- `control_actor.get_paper_tile_polygon` uses Epic's [`UPaperTileMap::GetTilePolygon`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMap/GetTilePolygon) to return the validated cell polygon, including rectangular, diamond, or hexagonal tile shapes.
- `control_actor.resize_paper_tile_map` uses Epic's [`UPaperTileMapComponent::ResizeMap`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMapComponent/ResizeMap) for bounded resizing of an owned component tile map; standalone asset references are intentionally not resized through this action.
- `control_actor.set_paper_tile_map_default_collision` uses Epic's [`UPaperTileMapComponent::SetDefaultCollisionThickness`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMapComponent/SetDefaultCollisionThickness), with finite thickness bounds and the API's explicit collision-rebuild option.
- `control_actor.configure_paper_tile_map_layer_collision` uses Epic's [`UPaperTileMapComponent::SetLayerCollision`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMapComponent/SetLayerCollision) with explicit layer collision, thickness, offset, and rebuild settings. Epic documents this as an owned-tile-map operation, so standalone asset edits remain guarded by the component's own ownership rules.
- `manage_asset.configure_tile_map_collision` uses Epic's [`UPaperTileMap::SetCollisionDomain`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMap/SetCollisionDomain), [`SetCollisionThickness`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMap/SetCollisionThickness), and `RebuildCollision` to persist map-level collision settings.
- `manage_asset.resize_tile_map` uses Epic's [`UPaperTileMap::ResizeMap`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMap/ResizeMap) and `RebuildCollision` to resize all layers while preserving cell contents where supported; dimensions are bounded to the documented 1-1024 range and persistence uses the safe asset-save wrapper.
- `control_actor.paint_paper_tile` uses Epic's [`UPaperTileMapComponent::SetTile`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMapComponent/SetTile) with [`FPaperTileInfo`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/FPaperTileInfo); `tileSetPath` resolves the tile set and `rebuildCollision` optionally calls `RebuildCollision` after the edit.
- `control_actor.fill_paper_tile_region` batches bounded `SetTile` calls for a rectangular region and optionally invokes `RebuildCollision` once after the batch; Epic documents both operations as requiring an owned tile-map component.
- `control_actor.set_paper_sprite_color` and `configure_paper_flipbook` use Epic's [`SetSpriteColor`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSpriteComponent/SetSpriteColor), [`SetPlayRate`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperFlipbookComponent/SetPlayRate), [`SetLooping`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperFlipbookComponent/SetLooping), and playback controls. Values are range-checked before applying them.
- `control_actor.configure_paper_character` uses Epic's [`APaperCharacter::GetSprite`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/APaperCharacter/GetSprite) plus `UCharacterMovementComponent` movement settings to assign a flipbook and configure playback, walk speed, and gravity.
- `manage_asset.create_sprite` uses Epic's [`UPaperSpriteFactory`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2DEditor/UPaperSpriteFactory) with `InitialTexture` and the existing Asset Tools creation path. It requires the optional Paper2D and Paper2DEditor modules plus a pre-existing `UTexture2D`.
- `manage_asset.create_tile_set` uses Epic's [`UPaperTileSetFactory`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2DEditor/UPaperTileSetFactory) with its optional `InitialTexture` and the existing Asset Tools creation path.
- `manage_asset.inspect_tile_set` uses Epic's [`UPaperTileSet`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileSet) getters to report the tile sheet, tile counts, tile size, spacing, margins, drawing offset, and authored texture dimensions.
- `manage_asset.configure_tile_set` uses Epic's `UPaperTileSet::SetTileSheetTexture`, `SetTileSize`, `SetPerTileSpacing`, `SetMargin`, and `SetDrawingOffset` setters with bounded pixel values and safe persistence.
- `manage_asset.configure_tile_set` also supports Epic's [`UPaperTileSet::SetBackgroundColor`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileSet/SetBackgroundColor), accepting finite linear color components and returning the value through `inspect_tile_set`.
- `manage_asset.get_tile_set_tile_uv` uses Epic's [`UPaperTileSet::GetTileUV`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileSet/GetTileUV) to return validated texture-space coordinates for a tile index.
- `manage_asset.configure_tile_set_tile_metadata` uses Epic's [`UPaperTileSet::GetMutableTileMetadata`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileSet/GetMutableTileMetadata) and [`FPaperTileMetadata`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/FPaperTileMetadata) to author the public per-tile `UserDataName` tag, including clearing it with an empty string.
- `manage_asset.get_tile_set_tile_xy` uses Epic's [`UPaperTileSet::GetTileXYFromTextureUV`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileSet/GetTileXYFromTextureUV) to convert finite texture-space coordinates to tile-grid coordinates with the API's optional rounding mode.
- `manage_asset.inspect_tile_set_tile_metadata` uses Epic's [`UPaperTileSet::GetTileMetadata`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileSet/GetTileMetadata) and [`FPaperTileMetadata`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/FPaperTileMetadata) helpers for per-tile user-data, terrain-membership, and collision readback.
- `manage_asset.get_tile_set_uv_from_xy` uses Epic's [`UPaperTileSet::GetTileUVFromTileXY`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileSet/GetTileUVFromTileXY) to return the top-left texture-space pixel coordinate for a validated tile-grid coordinate.
- `manage_asset.inspect_tile_map` uses Epic's [`UPaperTileMap`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMap) public map fields and getters to report dimensions, layers, scale, collision, and editor grid colors without mutating the asset.
- `manage_asset.configure_tile_map_visuals` writes Epic's public `UPaperTileMap` visual UPROPERTY fields for background/grid colors, multi-tile grid dimensions/offsets, pixels-per-unit, and layer/tile Z separation, with finite/bounded validation and safe persistence.
- `manage_asset.configure_tile_map_layer` uses Epic's [`UPaperTileLayer`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileLayer) setters for layer color, collision participation/overrides, and editor visibility, with bounded values and safe tile-map persistence.
- `manage_asset.inspect_tile_map_layer` uses Epic's `UPaperTileLayer` getters for layer name/dimensions, occupied-cell count, collision, render visibility, and color readback.
- `manage_asset.set_tile_map_cell` and `fill_tile_map_region` use Epic's [`UPaperTileLayer::SetCell`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileLayer/SetCell) with [`FPaperTileInfo`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/FPaperTileInfo) for validated standalone tile-map asset editing; region writes are bounded and collision rebuild is optional.
- `manage_asset.get_tile_map_cell` uses Epic's [`UPaperTileLayer::GetCell`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileLayer/GetCell), `FPaperTileInfo`, and [`UPaperTileLayer::GetTileTransform`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileLayer/GetTileTransform) to return the packed tile index, flags, source tile set, validity, and decoded transform for an in-bounds asset cell.
- `manage_asset.get_tile_map_tile_geometry` uses Epic's [`UPaperTileMap::GetTilePositionInLocalSpace`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMap/GetTilePositionInLocalSpace), `GetTileCenterInLocalSpace`, and [`UPaperTileMap::GetTilePolygon`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperTileMap/GetTilePolygon) to return bounded local-space position, center, and rectangular/diamond/hexagonal polygon geometry for a tile-map cell.
- `manage_asset.get_tile_map_tile_coordinates` uses Epic's [`UPaperTileMap::GetTileCoordinatesFromLocalSpacePosition`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/PAPER2UPaperTileMap/GetTileCoordinatesFromLocalSpacePosition) to convert a finite local-space position into tile-grid coordinates.
- Cell writes accept packed `FPaperTileInfo` flags in the documented 0–7 range via [`FPaperTileInfo::SetFlagsAsIndex`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/FPaperTileInfo/SetFlagsAsIndex), preserving Paper2D tile transforms for single-cell and region edits.
- `manage_asset.set_sprite_pivot` uses Epic's [`UPaperSprite::SetPivotMode`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSprite/SetPivotMode) and [`ESpritePivotMode`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/ESpritePivotMode__Type), validating documented named modes or finite custom texture-space coordinates before optional render/collision rebuild and safe save.
- `manage_asset.inspect_sprite` uses official [`UPaperSprite`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSprite) getters for source texture/region, pivot, collision domain, and collision thickness, returning numeric enum values without mutating the asset.
- `manage_asset.configure_sprite_source` uses Epic's [`UPaperSprite::SetTrim`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSprite/SetTrim) to edit trimmed state and source-region origin/dimensions, then optionally rebuilds and safely saves the sprite.
- `manage_asset.configure_sprite_collision` updates Epic's `UPaperSprite` collision UPROPERTY values (`SpriteCollisionDomain` and `CollisionThickness`) through the bridge's guarded reflection layer, then calls [`UPaperSprite::RebuildData`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSprite/RebuildData) and safely persists the asset. Epic exposes getters but no public collision-domain setter on `UPaperSprite`.
- `manage_asset.inspect_sprite_sockets` uses Epic's [`UPaperSprite::QuerySupportedSockets`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSprite/QuerySupportedSockets) to return socket names and component-socket types without mutating the sprite.
- `manage_asset.get_sprite_socket_transform` uses Epic's [`UPaperSprite::FindSocket`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSprite/FindSocket) to return the authored [`FPaperSpriteSocket`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/FPaperSpriteSocket) local transform for a named socket.
- `manage_asset.remove_sprite_socket` validates a named socket with Epic's `UPaperSprite::FindSocket`, removes it through [`UPaperSprite::RemoveSocket`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSprite/RemoveSocket), and safely persists the edited sprite.
- `manage_asset.validate_sprite_socket_names` uses Epic's [`UPaperSprite::ValidateSocketNames`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSprite/ValidateSocketNames) to normalize empty or duplicate socket names, then returns the resulting socket list and safely persists the sprite.
- `manage_asset.find_sprite_texture_bounding_box` uses Epic's [`UPaperSprite::FindTextureBoundingBox`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperSprite/FindTextureBoundingBox) to calculate tight source-region bounds at a validated alpha threshold without mutating the asset.
- `system_control.execute_python_script`, `execute_python_string`, and `execute_python_file` are explicit aliases over the secured Python execution path, matching Epic's Editor Python distinction between literal code and script files; file/code mode is validated before execution. File mode is confined to project-relative `.py` files, rejects traversal/symlink escapes, missing files, and files over the 1 MB execution limit. `configure_python_paths` applies validated paths to the live interpreter's `sys.path` as documented by Epic; `list_python_packages` reports up to 500 distributions from the embedded interpreter via `importlib.metadata`. `create_editor_utility_widget` and `create_editor_utility_blueprint` use Epic's Blutility factories and Asset Tools creation API through the embedded editor Python environment; `run_editor_utility` validates and invokes an existing asset through `EditorUtilitySubsystem.can_run`/`try_run`, while `inspect_editor_utility` returns availability and widget-tab state. Use the Python project setting for restart-persistent Additional Paths.
- `manage_asset.inspect_flipbook` uses Epic's [`UPaperFlipbook`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperFlipbook) readback APIs (`GetNumKeyFrames`, `GetNumFrames`, `GetTotalDuration`, `GetFramesPerSecond`, and `GetKeyFrameChecked`) to return playback metadata and each keyframe's sprite/frame run.
- `manage_asset.get_flipbook_socket_transform` uses Epic's [`UPaperFlipbook::FindSocket`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperFlipbook/FindSocket) to return a named socket's local transform at a validated keyframe index.
- `manage_asset.get_flipbook_sprite_at_time` uses Epic's [`UPaperFlipbook::GetSpriteAtTime`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperFlipbook/GetSpriteAtTime) and `GetKeyFrameIndexAtTime` to resolve the sprite/keyframe at a finite playback time with explicit end clamping.
- `manage_asset.get_flipbook_sprite_at_frame` uses Epic's [`UPaperFlipbook::GetSpriteAtFrame`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperFlipbook/GetSpriteAtFrame) for validated direct frame-index sprite lookup.
- `manage_asset.create_flipbook` uses Epic's [`UPaperFlipbookFactory`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2DEditor/UPaperFlipbookFactory) and [`FPaperFlipbookKeyFrame`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/FPaperFlipbookKeyFrame), validating each `spritePaths` entry and optional `frameRuns` before Asset Tools creation.
- `manage_asset.create_tile_map` uses Epic's [`UPaperTileMapFactory`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2DEditor/UPaperTileMapFactory) and its `InitialTileSet` seed to create a tile-map asset through Asset Tools; the optional `tileSetPath` must resolve to an existing `UPaperTileSet`.
- `manage_asset.add_flipbook_keyframe` and `set_flipbook_framerate` use Epic's [`FScopedFlipbookMutator`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/FScopedFlipbookMutator) to edit keyframes and FPS while preserving cache invalidation and safe asset persistence.
- `control_actor.configure_flipbook_loop` is the roadmap-compatible alias for the validated `configure_paper_flipbook` path and calls Epic's [`UPaperFlipbookComponent::SetLooping`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/Paper2D/UPaperFlipbookComponent/SetLooping).
- `manage_geometry.simplify_mesh_tool`, `remesh_tool`, and `boolean_tool` are explicit aliases over the existing GeometryScript simplify, voxel-remesh, and boolean handlers. `boolean_tool` validates and routes `operation` as union, subtract, or intersection; the aliases do not claim to activate Modeling Tools Editor Mode.
- `manage_geometry.get_mesh_section_data` uses Epic's [`UProceduralMeshComponent::GetProcMeshSection`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/UProceduralMeshComponent/GetProcMeshSection) and the public [`FProcMeshSection`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/FProcMeshSection) / [`FProcMeshVertex`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Plugins/ProceduralMeshComponent/FProcMeshVertex) buffers for section readback, including tangent direction and handedness.
...
- `manage_geometry.get_mesh_section_bounds` derives a section-local `FBox` from the official `FProcMeshSection` vertex buffer and returns its minimum, maximum, center, and extent.
...
`manage_geometry.unwrap_uvs` and `pack_uvs` are explicit roadmap-compatible aliases for the existing GeometryScript `unwrap_uv` and `pack_uv_islands` mesh operations.
...
The mesh-info response also reports the local-space bounds from Epic's [`Get Mesh Bounding Box`](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/GeometryScript/MeshQueries/GetMeshBoundingBox), including validity, min/max corners, center, and extent.

`system_control.create_geometry_collection` uses Epic's [`GeometryCollectionFactory`](https://dev.epicgames.com/documentation/en-us/unreal-engine/python-api/class/GeometryCollectionFactory) and `AssetToolsHelpers` to create and save a Geometry Collection asset. Epic's current Fracture Editor API exposes modal tools and editor mode types rather than a stable general-purpose fracture factory, so composition and fracture operations remain explicitly unclaimed until that API boundary changes.

`system_control.configure_geometry_collection` uses Epic's documented `GeometryCollection` Python properties (`mass`, `mass_as_density`, `enable_clustering`, `enable_nanite`, `support_ray_tracing`, `damage_threshold`, `max_cluster_level`, and `remove_on_max_sleep`). The bridge allowlists these properties and saves through `EditorAssetLibrary`.

`system_control.inspect_geometry_collection` reads the same documented properties with `get_editor_property`, returning the asset/class paths and scalar/array values while tolerating version-specific properties that are not present.

`system_control.add_geometry_to_collection` and `remove_geometry_from_collection` use Epic's [`GeometryCollectionSource`](https://dev.epicgames.com/documentation/en-us/unreal-engine/python-api/class/GeometryCollectionSource) struct and the documented `GeometryCollection.geometry_source` property. They compose collections from existing `/Game` assets and remove sources by their referenced asset path.

`system_control.configure_geometry_collection_component` uses Epic's [`GeometryCollectionComponent`](https://dev.epicgames.com/documentation/en-us/unreal-engine/python-api/class/GeometryCollectionComponent) API to configure the supported actor component settings: simulation, collision-driven damage, cluster group/maximum simulated level, linear ether drag, and collision profile.

`system_control.set_ui_scale` persists Epic's [`UUserInterfaceSettings::ApplicationScale`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UUserInterfaceSettings) and applies it live through [`FSlateApplication::SetApplicationScale`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Slate/FSlateApplication). `system_control.announce_accessible_string` uses [`UGameplayStatics::AnnounceAccessibleString`](https://dev.epicgames.com/documentation/en-us/unreal-engine/API/Runtime/Engine/UGameplayStatics/AnnounceAccessibleString); platform support remains engine/platform dependent.

The Python editor-utility actions use Epic's [`EditorUtilityBlueprintFactory`](https://dev.epicgames.com/documentation/en-us/unreal-engine/python-api/class/EditorUtilityBlueprintFactory) and `EditorUtilityWidgetBlueprintFactory`. Python command registration uses Epic's [`UICommandsScriptingSubsystem`](https://dev.epicgames.com/documentation/en-us/unreal-engine/python-api/class/UICommandsScriptingSubsystem) with `ScriptingCommandInfo`; registered delegates are retained in the live Python interpreter so they can be unregistered later in the same editor session.
...
### Lightmass and Precomputed Lighting
```cpp
// World-level Lightmass settings are stored on the active WorldSettings.
AWorldSettings* WorldSettings = World->GetWorldSettings();
FLightmassWorldInfoSettings& Lightmass = WorldSettings->LightmassSettings;
Lightmass.StaticLightingLevelScale = 1.0f;
Lightmass.NumIndirectLightingBounces = 3;
Lightmass.NumSkyLightingBounces = 1;
Lightmass.IndirectLightingQuality = 1.0f;
Lightmass.IndirectLightingSmoothness = 1.0f;
Lightmass.VolumeLightingMethod = VLM_VolumetricLightmap;
Lightmass.bUseAmbientOcclusion = true;
Lightmass.VolumetricLightmapDetailCellSize = 200.0f;

// Build after changing settings.
GEditor->Exec(World, TEXT("BuildLighting Preview"));
```

The bridge exposes these settings through Phase 29.3 actions, including
volumetric-lightmap density and memory controls, baked ambient occlusion,
indirect-lighting cache CVars, quality builds, and complete settings readback.
